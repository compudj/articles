...
list's tower---fall back to copying a larger region, a grace-period
drain, careful ordering in reader and updater, or declining it. Linux's
list has no coherent reverse walk: \code{\_\_list\_del()} publishes its
forward edge with a marked store, its back edge with a plain assignment,
so the chains never change as one event.
...
one store switches the set. Two transacted structures follow: a
bidirectional list whose chains stay mutually coherent, so a reader may walk
...
and to its new value after. But an \rcu reader is \emph{not} a transaction of any
kind---each load resolves independently, so a traversal spanning several slots may
...
read side is what RCU exists to keep cheap. A \pseudotxn's marker instead rides
...
made immune to it~\citep[\S5.3.5]{PaulEdwardMcKenneyPhD}, and pay in read-side
performance~\citep{JoshTriplettPhD}; or decline the operation.

\Cref{sec:onestore} looks at a library that declined the coherent form of the
operation, and it is the cleanest evidence available that the problem is real,
and it is not confined to a niche library. Linux's \rcu list --- the most widely deployed there is ---
...
assignment, and \code{\_\_list\_add\_rcu()} writes it \emph{after} the
release that publishes the node. Userspace \rcu's list has the identical
shape. The consequence is not a subtle ordering nit but a semantic one:
the two chains are changed by two stores and nothing makes them one
event, so there is an interval in which they describe different lists
--- and a removed node survives as a ghost until its grace period
elapses, so the \code{prev} view can trail the \code{next} view by that
long. No barrier fixes either problem. So both libraries ship forward
iterators and no \rcu-safe reverse walk, and Linux goes further, poisoning
the back pointer in \code{list\_del\_rcu()} so no concurrent \rcu reader can
follow it. Reverse traversal is not exotic --- the same libraries' non-\rcu
lists offer it as a matter of course --- and its absence from the \rcu
variants was a considered tradeoff rather than an inability: the poison that
rules out a reverse walk buys use-after-delete debugging on the forward one,
which was judged the more valuable of the two, and userspace \rcu mirrored the
kernel's interface. What no such tradeoff could have bought is a reverse walk
\emph{coherent} with the forward one, and when the kernel later did want to
read a \code{prev} pointer under \rcu it got the reverse step without the
coherence (\cref{sec:bidirexception}).
...
significant additional cost on \rcu readers in the absence of updates. Three
terms recur. The \emph{embedder} is the code that builds a data structure on
this facility; a \emph{writer} is a thread executing one of that structure's
updates; and a \emph{slot} is one word-sized, reader-visible location in the
embedder's own structure --- a list's \code{next} or \code{prev}, a skip list
tower's forward pointer --- which in practice holds a pointer. The facility
changes a set of slots at once and owns none of them: the slots, the tag that
marks them, and the exclusion between writers are all the embedder's
(\cref{sec:embedderowes}). Specifically:
...
\item \textbf{A \pseudotxn model} (\cref{sec:model}): a frozen set of
      ...
      including the property that an \rcu reader is not a transaction of any kind, and
      the weaker one that it does have: monotone resolution, never
      new-then-old (\cref{sec:monotone}). A reader that resolves a single slot
      is itself linearizable, taking effect at its own load; it is the
      traversal of several slots that is not a transaction
      (\cref{sec:guarantees}).
...
\item \textbf{A per-record tag scheme} (\cref{sec:tags}): while a commit is in
      flight a slot does not hold its value, but the address of the record that
      carries it, marked in the pointer's spare bits ---
      \code{(record\_address | tag)}. The marker therefore lives in the slot
      rather than in per-element state, which is what takes the reader's added
      loads to zero --- the test is on a register the reader already holds, and
      costs it only a predicted branch. Because a
      ...
      Stealing alignment bits is neither new nor a private convention: low-bit
      tagging was established practice by 1993~\citep{gudeman1993typeinfo}, and
      a standard facility for it is under review for
      C++~\citep{wg21p3125}.
...
\item \textbf{Measurements on the reader axis} (\cref{sec:measured}), scaling to
      ...
      strictly more, the reader pays 14--21\% for existence structures and
      27--50\% for RLU. A single writer with readers running pays about twice, and a profile
...
reader does not pay, and what the writer does. \emph{Existence
structures}~\citep{mckenney2015issaquah} are the nearest of the three: each
element names an \emph{existence group}, and one store to that group's state
word changes the membership of every element in it at once.

% ===========================================================================
\section{Background}
\label{sec:background}

This section fixes the vocabulary the rest of the paper needs and states the
guarantee it sets itself against. Readers familiar with \rcu will find little
new in \cref{sec:rcu} and \cref{sec:buildpublish} beyond the comparison that
closes the first; \cref{sec:onestore} is where the problem is stated, and
...
For a linked structure this is usually stated as a traversal guarantee in two
parts: a node that stays on the traversal path for the whole traversal will be
reached, and a node that stays off it for the whole traversal will not be. A
node inserted or removed while the traversal is in flight may be reached or
missed, and the reader is not entitled to say which.

What \rcu does not promise is anything about \emph{several} slots at once. Two
edges changed by two stores are two unrelated events to a reader, whatever order
they were written in --- the gap \cref{sec:onestore} exhibits on shipped code. A
\pseudotxn narrows it: the slots of one commit resolve monotonically, never
new-then-old (\cref{prop:monotone}). One commit is one instant, so this asks
nothing of the writer beyond committing once, and nothing of the reader beyond
a dependency chain --- the chain's shape does not matter, and a walk around a
cyclic structure is served as readily as a walk down a list. A reader that
reaches two slots independently, with no chain between them, must order those
loads itself. Splitting one edit across \emph{several} commits is a different
question, with its own benefits and its own conditions on the traversal, and
\cref{sec:publishorder} takes it up.
...
not unusable so much as pointless: compilers implement it by promoting it to
\code{acquire}~\citep{boehm2008memorymodel,batty2011cppconcurrency}, so it costs
what an acquire costs and buys nothing over writing one. Userspace \rcu
therefore relies on the dependency itself --- and where a toolchain threatens
that dependency the ordering is given up rather than defended: an arm64 kernel
built with \code{CONFIG\_LTO} overrides \code{READ\_ONCE()} to acquire, because
link-time optimization may turn an address dependency into a control
one~\citep{linuxarm64rwonce}.}

A traversal is a chain of exactly these dependencies. Each hop's address is
computed from the value the previous hop returned, so the ordering that makes a
single \code{rcu\_dereference()} safe extends along the whole walk, and a reader
that follows pointers has it without asking for it. That is a small fact with a
large consequence: it is what \cref{prop:monotone} will need, so an ordinary
descent of a tree or walk down a list already satisfies that property's
precondition, and pays nothing to do so.
...
reader can reach them}. Because they are unreachable, this phase owes readers
nothing: no ordering is needed on their behalf, no barrier is paid, and an
operation that fails here has published nothing and can free its work
immediately with no grace period. Nor is it confined to one thread --- a large
addition may be built in parallel, and readers are none the wiser. Whatever
ordering the builders need among themselves is discharged before the publish,
and the release store that publishes carries everything sequenced before it.
...
no lock and paid no barrier.\footnote{One near-extinct exception is worth
naming: the 68020, 68030 and 68040 carried \code{CAS2}, a compare-and-swap over
two \emph{independently addressed} operands~\citep{motorola1992m68000prm}. What
later architectures kept was double-\emph{width} compare-and-swap over one
contiguous pair, which is a different thing, and \code{CAS2} was fixed at two
locations where a \pseudotxn is not.}

Hardware transactional memory does not dissolve the constraint, though it is the
first place to look. A transaction can publish a whole multi-word update atomically to
a lockless reader~\citep{ramadan2007txlinux}, but a commodity implementation may
abort one for reasons the program neither controls nor predicts, so every user
needs a non-transactional fallback --- and that fallback meets exactly the
problem this paper is about. Guaranteed progress does exist: s390's
\emph{constrained} transaction always commits and needs no abort handler. Its
price is at most 32 instructions within 256 bytes, with many instructions barred
outright --- tight enough that IBM documents the facility at assembler level and
ships an assembly-time checker for the rules~\citep{cebula2013s390tef} --- and
s390 is not commodity hardware. We assume only the single-word store and
compare-and-swap that hardware provides everywhere.
...
\paragraph{The kernel's narrow exception proves the rule.}
\label{sec:bidirexception}
...
for it. That is why the fix has to be a mechanism rather than a barrier. It is
not why the two libraries withheld the operation --- Linux traded the reverse
walk for the poison's debugging value, and userspace \rcu mirrored the kernel's
interface --- but it is why neither could have offered a \emph{coherent} one had
it chosen to.
...
\label{def:resolve}
To \emph{resolve} a slot is to read it through the facility's read accessor,
which returns a value from the embedder's own domain rather than whatever
encoding the slot may currently hold. Resolution is the only read this model
speaks about: a bare load of a slot participating in an uncommitted \pseudotxn
may yield an encoding rather than a value, and nothing here constrains it.
\end{definition}
...
% ANCHORED, not a [tb] float, for the reason documented at fig:readcost and
% tab:comparison.  This table is introduced by the colon above it and by nothing
% else -- it carries no \cref anywhere -- so as a float it read correctly only by
% the accident of landing on the same page as its own introduction.  Anchoring
% makes that structural instead of lucky.  \captionof numbers it as a table.
...
\bottomrule
\end{tabular}
\captionof{table}{The model is the journaling sense of \emph{transaction}, not the
...
read side is precisely what this facility leaves cheap.
% -----------------------------------------------------------------------
...
The whole reader-visible semantics of the model is one function --- what a slot
resolves to (\cref{def:resolve}) --- and it has three cases.

\begin{itemize}
\item A slot in no \pseudotxn resolves to its own contents.
\item A slot carrying a record of a frozen but not yet committed \pseudotxn
      resolves to that record's $o$.
\item A slot carrying a record of a committed \pseudotxn resolves to its $n$.
\end{itemize}

The cases are exhaustive and the state space is closed: the tidying that follows
a commit returns each slot to the first case, its contents now $n$, so a slot
leaves the third case only by ceasing to be transacted at all. There is no state
in which a slot resolves to neither value, and none in which part of a record
set has moved to $n$ while part is still $o$.

The four properties below restate this in the terms the rest of the paper uses.
\Cref{prop:atomicity} is the transition from the second case to the third and
\cref{prop:notearing} is the absence of any fourth case; the remaining two say
where that transition happens and how far it reaches.
...
\label{prop:lp}
The commit is a linearizable~\citep{herlihy1990linearizability} \emph{write
operation}: it takes effect atomically at a single instant, the commit step,
which is its lone linearization point. The staging that precedes it and the
tidying that follows change no value a reader can resolve --- the embedder's own
writer sees both, and a bare load of a slot the \pseudotxn has a record on
would see an encoding rather than a value (\cref{def:resolve}) --- so neither is
...
linearizes is therefore a precise and limited claim, and it is a claim about the
commit alone. It does not reach the slots a writer read on its way to staging
one: those reads are not part of the commit step, and nothing in
\cref{prop:lp} makes a read-then-commit bracket one atomic operation. Still less
does it reach a \emph{reader's} traversal, which is not a transaction of any
kind (\cref{sec:nonguarantees}). Serializability is claimed nowhere in this
paper.
...
publishing into $A$ and then into $B$ with two stores, where a node can be
reachable in $A$ before it is reachable in $B$. A single update-side lock over
both structures does not close that window: it excludes other \emph{writers},
and the observer here is a reader, which takes no lock and sees the two
publications as two events however the writers were serialized. The unit of cross-structure atomicity is exactly
one \pseudotxn: two commits are two linearization points, the same as two
unrelated \rcu structures. This property is also what the facility pays for
downstream: structures that may be spanned together need not share a pointer
encoding, so the proxy marker cannot be a constant chosen once
(\cref{sec:pertag}).
\end{property}
...
genuinely are two publications (\cref{fig:skew}). Callers are left to tolerate the skew, which
usually means the structures grow a validity flag, or a lock, and the reason
\rcu was chosen erodes.

A \pseudotxn takes the slots of both structures into one write set: the index
...
% ---------------------------------------------------------------------------
\textbf{An \rcu reader is not a transaction of any kind.} It is a sequence of
...
It has no read set of its own --- the read set a \pseudotxn may carry is the
writer's, not a reader's --- and nothing begins or commits on the read side: the
...
\textbf{Read validation is opt-in, and this facility does not enforce it.} The
model admits per-slot read validation: a slot an operation depends on may be
entered into its read set. What turns that declaration into a check is an
install compare-and-swap, which the \sw facility does not have
(\cref{sec:exclusion}) --- under writer exclusion there is no peer to detect, so
\cref{def:record}'s $o$ serves to \emph{resolve} a proxy rather than to verify
memory. Marking reads as well as writes is what would serialize the two against
each other, and it is the \mw facility of the companion paper that pays for the
detection this one declines.
...
not what it must have held throughout (\cref{sec:modellimits}).
% -----------------------------------------------------------------------
% NOTE (drafting; retained deliberately -- see README framing rule).
...
back. A reader that resolves several slots of one \pseudotxn \emph{along a
dependency chain} --- each hop's address derived from the value the previous hop
loaded --- therefore observes a monotone sequence --- $o \ldots o, n \ldots n$
--- and \textbf{never $n$ then $o$}. A reader may lag a commit; it never
regresses across one. Hops that are not so chained carry no such edge, and the
reader must order them itself; the scope below states when and how.
\end{property}
...
with it, and the rule is general: \emph{monotonicity passes from the deciding
word to a reader's sequence of resolutions exactly as far as that reader's
loads are ordered against one another.} Monotonicity of the deciding word is
unconditional --- it is one word, stored once --- but a sequence of loads
inherits it only across pairs that carry an ordering edge, and the reader is
responsible for every pair that does not. There are two ways to hold such an
edge, and only two: an address dependency, or an acquire load.

A \emph{dependency-chained} traversal --- each hop's address derived from the
value the previous hop loaded --- has the first for free on every architecture
\urcu supports, as does any build using the default C11 dereference. That is
the common case and it is why the precondition is usually invisible: an
ordinary walk down a list or a tree satisfies it without trying. A reader that
hops \emph{without} that chain has neither edge until it supplies one. Two
shapes reach a slot with no chain to the last: re-reading a pointer it cached
earlier, and reaching two slots independently --- two separate structures, or
two sibling subtrees of one, as a depth-first walk without parent pointers
reaches them. Under \code{-DURCU\_DEREFERENCE\_USE\_VOLATILE} on weakly-ordered
hardware such a reader's two loads may be reordered, and it can observe the new
value on the later slot and the old on the earlier one.
...
snapshot, and a traversal touching several slots may straddle a commit, how can
a multi-slot traversal ever be relied on? \emph{Correct} will not settle it,
because it is not one criterion: a definition strict enough to rule this
facility out rules plain \rcu out with it, since an \rcu reader may likewise
act on a value a writer has already replaced. The question that can be settled
is narrower --- what does a traversal need in order to reach a conclusion it may
act on, and does it get it? The answer is not that snapshots are
...
\emph{Usually} carries the weight in that sentence. The trade is available to a
reader whose slots arrive in one fixed order and whose hops carry that order ---
the dependency chain of \cref{sec:monotone} --- and to no other. A reader that
reaches two slots independently presents no read order for the writer to agree
with, and a structure with no single traversal order, such as any cycle or a
list read in both directions, offers none to reverse. Both cases return below.

\paragraph{The rule.} Suppose a reader resolves $k$ \emph{distinct} slots in a
fixed order $s_1, s_2, \ldots, s_k$, and the writer splits the edit across
several \pseudotxns rather than one. Then:
...
Distinctness is a real condition and not bookkeeping, and it is a condition on
slots rather than on the shape of the path. A reader may return to a node it has
already visited without resolving any slot twice: the descend-then-ascend reader
below does exactly that, reading a parent's child link on the way down and a
child's parent link on the way back up, which are two different words. What the
rule cannot survive is resolving one slot twice. That puts one slot at two
positions while a slot has one publish instant --- the rule would have to
publish $s_1$ last and also before $s_1$'s second appearance --- so for such a
reader it cannot be stated at all. Resolve the ambiguity either way and
new-then-old is admitted --- resolve $s_1$, then $s_2$ after $t_2$, then $s_1$
again before $t_1$ --- which is the one observation the rule exists to prevent.
This bounds the \emph{ordering} rule and nothing else. \Cref{prop:monotone} is
untouched by it: under a single \pseudotxn the deciding word is written once, so
a slot that has resolved new resolves new on every later visit, and a reader
that resolves the same slot repeatedly is as monotone as one that does not.
...
Whether that harms anyone is the caller's question rather than the structure's.
A lookup whose answer may be either the old value or the new is unharmed by
seeing them out of order; one whose answer must agree with a decision already
taken is not. Where it does harm, the reader cannot repair it unaided --- it
cannot tell a stale load from a fresh one --- but it need not build a snapshot
to be told. A seqcount around the traversal detects an interfering update after
the fact and retries, which is how Linux's dentry cache guards a path walk
against a concurrent rename~\citep{linux2026dcache}; a per-element lock
prevents the interleaving instead, as \rcu's early System~V IPC work
did~\citep{Arcangeli03}. Both are real remedies, and both are paid for on the
read side on every traversal, whether or not a writer ever appears.

\emph{This is the observation that forces those remedies, and it is the one the
model already forbids.} \Cref{prop:monotone} rules it out by construction, because the
...
a key still present at a location it has departed from. When those are tolerable, and how
a structure that cannot tolerate them compensates for them, is the subject of later
...
\paragraph{Why this is safe, and where it stops.} The argument above used two
orders without saying where either comes from: the order in which the writer
publishes the two edges, and the order in which the reader resolves them.
Neither is automatic. The writer's is free here: under exclusion
(\cref{sec:exclusion}) the commits are two release stores issued in program
order by one thread. The reader's is the one that needs care, and it is exactly the precondition of \cref{sec:monotone} --- the
reader's loads must be genuinely ordered, not merely written in that sequence.

They are, for the reason \cref{sec:rcu} gave: the reader
...
\paragraph{When the rule is needed at all.} It is worth being concrete about
this, because a great many tree edits never reach for the rule. Inserting a \emph{new} node is the
...
build-and-publish cannot reach: restructuring a node that is \emph{pinned}. A
node is pinned when its address is held somewhere the writer cannot reach to
update --- an application handle, a cache in another subsystem, a value some
unrelated structure has already stored away. Copying is \rcu's usual escape from
an in-place edit: rebuild the node in private and swap it in with one store. But
a swap is complete only if every reference to the old node is redirected, and a
writer can redirect the references it owns and no others. One reference it does
not own is enough.

Widening the \pseudotxn does not dissolve this, and it is worth saying why. A
commit changes \emph{slots}, and a slot is a word the facility has been told
about (\cref{sec:embedderowes}); a handle in an application variable, or a
pointer held by a structure this facility does not manage, is not a slot and
cannot be enrolled in a record set. A wider commit absorbs more of the
structure's own pointers, which is often worth doing, but it never reaches the
references the writer does not own.

So the node cannot move, and its edges must change beneath it. A new node could
carry its parent pointer into visibility because it was assembled in private; a
pinned node cannot, so its parent pointer has to be
...
\paragraph{Which remedy applies.} The choice is decided by the structure's
traversal orders. The
...
The limits should be equally clear. This is an embedder's discipline, not a
...
--- is never be told that time ran backwards. The descend-then-ascend reader
above is the simplest instance. Having descended into $E$ and found it under
$Q$, it needs \code{E->parent} not to answer $P$. It is not asking what the tree
looked like while it walked, and it would not know what to do with the answer;
it is asking that the one edge it reads next not contradict the path it took to
reach the question.
...
deadline whatever the write rate. A \pseudotxn does not change shape for those:
it moves cost to the writer either way, which suits a read-dominated workload
and equally a bounded read path, and is the wrong trade only where commit
throughput is the scarce resource.} Its entire contract is that a reader pays
...
this paper has already said why: \cref{sec:intro} names copying the enclosing
region as one of \rcu's remedies for an update that spans several words. \rcu
provides a free-to-readers snapshot: copy. A writer that builds a new version where
...
reader is the one party \rcu exists to keep cheap.
...
\emph{reader}, and \cref{fig:readclass} says it: 27--50\% against RLU, and
...
% the READER AXIS and only there. fig:readclass reports what the stronger
...
\emph{proxy} rather than the target directly. A slot in that condition is
\emph{parked}, and stays parked from the moment the writer points it at the
proxy until the tidying after the commit writes its direct target back;
installing a proxy and parking a slot are the same event, named from the
writer's side and the slot's side. A proxy carries
...
$\mathit{old}$ while the selector is $0$, and $\mathit{new}$ once it is $1$. Two
states are what this facility needs, not all a selector could carry. A third is
the obvious extension and the companion paper's \mw facility has one, for
\emph{abort}: a commit that contends can fail, and a reader must be able to see
that it did. Under exclusion a commit cannot fail, so the state cannot arise
here --- which is why the shared status vocabulary reduces to a boolean in this
paper (\cref{sec:exclusion}). A many-valued selector is a different extension
again, and \cref{sec:varselector} sets out what a generation counter would buy
...
precondition of the mechanism, not a caveat on it --- and an embedder that
cannot meet it is not out of options. The companion paper's \mw facility
realizes the same model with the precondition lifted, and pays for the
difference in exactly the machinery this one omits.
...
The mutation is driven by the embedder, whose writers perform the structure's
updates. The embedder owns the whole lifecycle---all four steps below---and the
obligations of \cref{sec:embedderowes}; "builder" would name only the first
step.
\begin{enumerate}
...
changing any value, and settle removes it without changing any value. Settle
\emph{is} the uninstall, which is why there is no fifth step named for it. The
\code{call\_rcu()} it ends with is not a second action of the same kind: it hands
the proxies and the group to \rcu and returns, and the free happens a grace
period later on the reclaim side of build--publish--reclaim. The lifecycle names
what the writer does, and the writer is finished at settle.
...
the lifecycle --- and through the states either side of it, so six rows for four
steps --- alongside what a concurrent reader resolving $s$ actually obtains.

\begin{table}[tb]
\centering
\caption{One transacted slot $s$ through the lifecycle, bracketed by the state
before it begins and the state after it ends. The reader's column
...
The facility is deliberately incomplete: it supplies the \fliplatch, and leaves three
...
\item \textbf{Reclamation.} After settle has uninstalled every proxy, the
...
This imposes a contract on the embedder, and it is unforgiving:
...
since a bare load of a parked slot yields a tagged record address rather than
the value that slot resolves to.
...
So each record --- one \record{s}{o}{n} triple naming one slot
(\cref{def:record}) --- carries its own \code{proxy\_tag}, supplied by the
writer at the moment it adds that record to the \pseudotxn it is staging.
Heterogeneous slots therefore share one facility, each resolved through its own
tag.
...
The tag is the whole of the facility's presence in a reader's world, absent a
concurrent commit on the slot being read. There is no
...
While a slot is parked the reader pays more, and this is where it pays: it
follows the tagged pointer to the proxy and reads the selector. That is the
facility's entire charge on the read side, and it is transient by construction
--- it lasts from install to settle, on that slot and no other.
...
\Cref{fig:listinsert,fig:listremove} trace the two operations through that
lifecycle, each showing the reader's view above the updater's. They differ in
one respect worth the second diagram: an insert reclaims only the transaction's
own descriptor records, while a remove also retires a list node, and that node's
own pointers must outlive the commit so that a reader already standing on it
reads through to the neighbours it no longer sits between
(\cref{sec:reclaim}).

\input{fig-list-insert}

\input{fig-list-remove}
...
At every instant, for any two nodes that are both members of the list, the links
are mutual inverses: \code{a->next == b} implies \code{b->prev == a}. No
operation exposes a window in which one direction sees the change and the other
does not, so every resolution reads from a state in which the two chains agree.
\end{property}
...
can hold \code{A} after \code{D} is gone from the tail, reading
...
% writers are the multi-writer facility's story and the evaluation paper's.
%
% Restricting to the reader axis is what admits the cross-scheme comparison in
% fig:readclass: on this axis the comparison is sound in the direction that
...
192 workers, so no measurement below places two threads on one core; the
one-writer sweep therefore stops at 191 readers. Userspace \rcu's
...
\paragraph{Reads pay no extra load for the \pseudotxn layer.} \Cref{fig:readcost}
gives the read ceiling with no writer running, in millions of node visits per
second, both engines walking forward twice.

% NOT a float, deliberately. As a [tb] float this was placed at the top of the
% page the section starts on -- above the section heading, reading as belonging
% to sec:reclaim -- and \FloatBarrier did not hold it there. It is small, and it
% belongs beside the sentence that introduces it, so it is anchored.
% \captionof (caption, via subcaption) numbers it as a figure.
% Linear on both axes, deliberately: the claim is that the two curves coincide,
% and a log y would compress exactly the region where a gap would show.
\par\medskip\noindent\begin{minipage}{\linewidth}
\centering
\begin{tikzpicture}
\begin{axis}[
  width=0.92\linewidth, height=5.2cm,
  xlabel={Readers}, ylabel={Millions of node visits/s},
  xmin=0, xmax=196, ymin=0, ymax=160000,
  scaled y ticks={base 10:-3}, ytick scale label code/.code={},
...
\addplot[mark=*, mark size=1.5pt, thick, color=blue!65!black]
  table[x=readers, y=txn, col sep=comma] {data/fig-readcost.csv};
\addlegendentry{\code{txn\_sw\_list}}
\addplot[mark=square, mark size=1.5pt, thick, color=black!55, dashed]
  table[x=readers, y=rcu, col sep=comma] {data/fig-readcost.csv};
\addlegendentry{\code{rculist}}
\end{axis}
\end{tikzpicture}
\captionof{figure}{Read ceiling, no writer running, \emph{both engines walking
forward twice}. The two elements are byte-identical --- 24 bytes, two pointers
and an \code{int} --- so there is no footprint difference and no access-pattern
difference between the curves, and what separates them is the resolve alone.
They coincide: the widest ratio over the sweep is 1.029 and the narrowest 1.006,
against a run-to-run drift of about 0.8\%. Both axes are linear, so the absence
of a gap is the result rather than an artifact of compression.}
\label{fig:readcost}
\end{minipage}\par\medskip
...
one reader to 192, or 97.9\% of ideal, and \code{rculist} gains 187, or 97.4\%.

That the two agree is the point, and the construction is what makes it a fair
statement. The curves are not merely close; they are running the same traversal
...
\Cref{fig:readclass} places the same traversal against the schemes that also
...
\begin{axis}[
  ...
  xlabel={Readers}, ylabel={Millions of node visits/s},
  xmin=0, xmax=196, ymin=0, ymax=162000,
  scaled y ticks={base 10:-3}, ytick scale label code/.code={},
  yticklabel={\pgfmathprintnumber{\tick}\,k},
  xtick={0,32,64,96,128,160,192},
  legend pos=north west, legend cell align=left,
...
\addplot[mark=*, mark size=1.5pt, thick, color=blue!65!black]
  table[x=readers, y=txn, col sep=comma] {data/fig-readclass.csv};
\addlegendentry{\code{txn\_sw\_list}}
\addplot[mark=triangle, mark size=2pt, thick, color=teal!70!black]
  table[x=readers, y=existence, col sep=comma] {data/fig-readclass.csv};
\addlegendentry{Existence}
\addplot[mark=square, mark size=1.5pt, thick, color=orange!80!black]
  table[x=readers, y=rlu, col sep=comma] {data/fig-readclass.csv};
\addlegendentry{RLU}
\addplot[mark=diamond, mark size=2pt, thick, color=purple!70!black]
  table[x=readers, y=mvrlu, col sep=comma] {data/fig-readclass.csv};
\addlegendentry{MV-RLU}
\end{axis}
\end{tikzpicture}
\captionof{figure}{Read ceiling, no writer, all four walking forward then
backward --- the operation they share. Every curve is linear in the reader
count, so the separations are per-dereference constants rather than scaling
collapses: at 192 readers the transacted list leads existence by 1.20, RLU by
1.50 and MV-RLU by 3.17. Existence is measured in its best per-element layout,
carrying only its tagged word (a 32-byte element) with the writer-only state
segregated; charging it for the reference implementation's 136-byte element
would measure a struct layout rather than the design. MV-RLU runs its
global-clock configuration, which needs no calibrated per-machine constant.}
\label{fig:readclass}
\end{minipage}\par\medskip
...
\paragraph{The writer pays about twice.} \Cref{fig:writecost} measures the same
list with a single writer and readers running concurrently.

% Anchored, for the same reason as fig:readcost above.
\par\medskip\noindent\begin{minipage}{\linewidth}
\centering
\begin{tikzpicture}
\begin{axis}[
  width=0.92\linewidth, height=5.2cm,
  xlabel={Readers}, ylabel={Millions of updates/s},
  xmin=0, xmax=196, ymin=0, ymax=15.6,
  xtick={0,32,64,96,128,160,192},
...
\addplot[mark=square, mark size=1.5pt, thick, color=black!55, dashed]
  table[x=readers, y=rcu, col sep=comma] {data/fig-writecost.csv};
\addlegendentry{\code{rculist}}
\addplot[mark=*, mark size=1.5pt, thick, color=blue!65!black]
  table[x=readers, y=txn, col sep=comma] {data/fig-writecost.csv};
\addlegendentry{\code{txn\_sw\_list}}
\end{axis}
\end{tikzpicture}
\captionof{figure}{One writer, with concurrent readers: update throughput, each
side under an uncontended writer mutex. Both reclaim through \code{call\_rcu()}
and neither changes \rcu flavor, so the separation isolates the \pseudotxn
layer. Note the collapse between one reader and eight, which both sides suffer
and the cheaper writer suffers more --- plain \rcu falls from 14.25 to 4.02
while the transacted writer falls only from 7.12 to 3.05, which is why the
ratio is 2.00 at one reader, 1.31 at eight, and 2.30 by 191.}
\label{fig:writecost}
\end{minipage}\par\medskip
...
is not the design that answers them; they belong to the multi-writer facility and
...
The cross-scheme figures in \cref{fig:readclass} are read-side only, and are not
...
--- and the guarantees differ, so a write-side ranking across these curves would
compare quantities that are not commensurable. What the reader axis supports is
the narrow claim made above: what the stronger reader guarantee costs the
reader.
...
\cref{fig:readcost} is against \code{rculist} alone. Separating those costs needs
...
\textbf{No reader snapshot.} An \rcu reader is not a transaction of any kind
...
layer of resolved-but-still-present flip-latches. Settle exists to make the cost
...
% ANCHORED, not a [tb] float, for the reason documented at fig:readcost: as a
% float this landed at the top of the page the section starts on, ABOVE the
% section heading, with the tail of the previous section beneath it -- reading
% as a table belonging to that section rather than to this one.  It belongs
% beside the sentence that introduces it.  \captionof numbers it as a table.
\par\medskip\noindent\begin{minipage}{\linewidth}
\centering
\footnotesize
...
\bottomrule
\end{tabular}
% CAPTION LENGTH IS LOAD-BEARING, do not let it grow back. This caption was once
% 465 words and duplicated nearly all of sec:costwalk below it; a [tb] float
% whose caption exceeds a page cannot be set anywhere, so LaTeX deferred it past
% the bibliography. It now says only what the table needs to be read on its own,
% and the argument lives in sec:costwalk, where it belongs and already was.
\captionof{table}{Where each design pays for multi-pointer atomicity, on the read-side
fast path, per dereference. \emph{Extra loads} counts memory references over and
above the load the reader must issue anyway; \emph{extra branches} counts
conditional branches. Both are measured, not read off the sources
(\cref{sec:costwalk}). MV-RLU's counts are floors rather than constants: its
dereference reads the object header unconditionally, with no counterpart to
RLU's pure-reader bypass, and may then walk a version chain whose length the
reader does not control. \Cref{sec:costwalk} discusses what each column means
and what it omits.}
...
Two things the table does not single this design out for, before the ones it
does. Each design publishes with a single store, to the shared word named in the
table's last column: one store is what publication costs everywhere here, not
only here. And the branch column is 1 for three of the four rows, this one among
them --- the honest difference is in the loads column, not the branches column.

The two count columns are measured rather than read off the sources. Hardware
counters over a single-reader traversal, differenced against a longer run so
that startup cancels, give the marginal loads and branches per node visit; the
figure quoted is each design's count minus plain \rcu's, since the table counts
what a traversal pays \emph{over} what it must pay anyway. Every entry survives
that check --- this work at exactly zero extra loads and one extra branch,
existence at one and one, RLU's dereference at one --- and MV-RLU exceeds its
floors widely, at ten extra loads and fifteen extra branches.
...
\Cref{tab:comparison}'s existence row names what the mechanism \emph{requires},
which is the one word the reader tests. A reader who consults the reference
implementation will find considerably more embedded beside it --- group linkage,
a reclamation head, callbacks and the writer's lock, 112 bytes in all, of which
only the 8-byte word is ever read on a traversal. That surplus is an
implementation choice rather than a property of the design, and being separable
is what makes the effect measurable at all.

The cost is then \emph{density} rather than size: what matters is how much of
...
That progression is why \cref{fig:readclass} reports existence at 1.14--1.21
...
% concept costs when implemented well, which is why fig:readclass uses the
...
guarantee classes and mean little. What \cref{fig:readclass} does compare is the
...
set is atomic; an \rcu reader is not a transaction of any kind, and a traversal
spanning several
...
reader, and the reader is what \rcu exists to keep cheap.
...
status vocabulary---and, at the pinned commit, one descriptor and one status
word---an embedder that outgrows exclusion changes which discipline a slot
records under, not how it reasons. The programming model the two facilities share, and the
