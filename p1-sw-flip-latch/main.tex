...
An STM does not owe its caller that discipline, and the difference is the whole
bargain of this paper in miniature. Its read barrier sits on \emph{every} read,
so a search run inside a transaction is reconciled against the write set by the
same machinery that reconciles an edit, and the obligation never reaches the
caller. Here the traversal is plain \rcu, uninstrumented, indistinguishable from
a read outside any \pseudotxn{} --- which is why the reader carries none of the
per-element state and none of the read-path work \cref{sec:costcomparison}
charges the alternatives. Read-your-own-writes covers the slots a writer
records, and stops there. Naming the victims first is what the writer pays so
the reader can pay nothing.
