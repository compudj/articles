...
through the ordinary \rcu accessors and edit in the same bracket --- those
resolve the committed structure, not this \pseudotxn's pending one, so an edit
can be handed a node an earlier edit has already displaced. The wrappers take
the cheaper route where it is available: the caller's initial lookup runs on the
ordinary accessors, before the bracket, and only the neighbour links the edits
touch are read transactionally. No shape of update is ruled out by this. The
...
exclusion (\cref{sec:exclusion}) nothing outside the bracket can change a slot,
so a transactional walk has only the writer's own pending edits to reconcile;
...
concurrent the question is real, and the answer is the opt-in per-slot read
validation of the model (\cref{sec:model}), which enters a slot into the
transaction's read set to be checked at the commit point. It is opt-in and
per-slot precisely so that an operation pays for the dependencies it actually
has rather than for every pointer its traversal crossed. The companion paper
takes that up.
