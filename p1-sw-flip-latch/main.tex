...
% ---------------------------------------------------------------------------
\TODO{Re-pin: Sec. 6.3 now describes the pprev-out-of-txn hlist (urcu-txn-dev
09a8b796), which is NOT in the current pin cefb0414. That commit is UNPUSHED (it
sits on unpushed blob commits, so its hash is not yet stable). BLOCKS
submission: once it is pushed and stable, move the pin below to it and update the
date. Until then the prose is ahead of the pin.}
