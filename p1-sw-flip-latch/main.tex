...
along a dependency chain of any shape --- including one that closes a cycle,
where there is no fixed order to publish against and ordering tricks have
nothing to work with (\cref{sec:publishorder}). A reader that instead reaches
...
Distinctness is a real condition and not bookkeeping. A traversal that returns
to a slot it has already resolved --- a walk that closes a cycle --- puts one
slot at two positions, and a slot has one publish instant: the rule would have
to publish $s_1$ last and also before $s_1$'s second appearance, so for such a
...
a slot that has resolved new resolves new on every later visit, and a traversal
that closes a cycle is as monotone as one that does not.
