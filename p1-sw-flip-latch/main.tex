...
new-then-old (\cref{prop:monotone}). Because what decides that is a single
written-once word rather than an order the reader must be walking in, it holds
along a dependency chain whatever the chain's shape --- including one that
resolves a slot twice, and including a walk around a structure for which no
fixed publish order exists at all, where the ordering tricks of
\cref{sec:publishorder} have nothing to work with. A reader that instead reaches
two slots independently must order those loads itself.
