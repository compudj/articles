...
work in this series develops two ways to do so, each spending the read side's
defining property to buy it. A \emph{seqcount} around the commit lets a reader
notice that a mutation intervened and retry: torn-free, but only
obstruction-free, since a sustained stream of writers can starve the retry. A
\emph{validating transaction} folds the reader's loads into the commit's
conflict set --- the loads ``act as writes'', and the read is accepted only if
every slot it touched still holds what it returned --- torn-free, but it aborts
and retries under contention. Both forfeit the wait-free read that is the
baseline's whole point; the wait-free way to snapshot many words is to copy them
behind one pointer, which is the cost this engine exists to avoid. That the base
model declines to choose among these is the deliberate weakening of
\cref{sec:whyweakening}, not a shortfall.
% -----------------------------------------------------------------------
% NOTE (drafting; retained deliberately -- see README framing rule).
% The two snapshot approaches are ONGOING work, deliberately NOT pinned: the
% seqcount reader is design-plan-stage in the rcu-txn-blob layer (urcu-txn-dev
% 98c7f2f7 etc.), and the validating-transaction reader is the concurrent
% engine's urcu_txn_load_validate() (folds a read into the commit's conflict
% set; rcu-txn.h). Hint only -- do not describe either as shipped in P1, and do
% not pin them. The honest cost is verified from the blob design note: both
% forfeit wait-free reads (seqcount obstruction-free, writers can starve;
% validating-txn aborts/retries), and single-pointer COW is the wait-free
% alternative. That trilemma -- wait-free XOR torn-free for a flat multi-word
% value, COW being the wait-free corner -- is a legitimate defensive disclosure
% and worth stating even though the mechanisms are not P1's.
% -----------------------------------------------------------------------
