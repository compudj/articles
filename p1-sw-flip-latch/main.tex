...
arise: the one thing that could invalidate a recorded old --- an earlier edit in
the writer's own bracket --- is exactly what read-your-own-writes reconciles
...
(\cref{sec:fliplatch}): the tagged object a slot points at while the transaction
is live. Proxies and their group are typically allocated as a single backing
block with one \code{rcu\_head}, so a transaction costs one allocation and one
...
companion paper takes up the \mw engine, which realizes the same model for
...
status vocabulary, an embedder that outgrows exclusion changes which engine it
links, not how it reasons. The programming model the two engines share, and the
...
\item[\code{rcu-txn-status.h}] The commit-status enum
      ...
      commit the same way whichever engine it drives.
\end{description}
...
companion paper, \code{rcu-txn.h} is its transaction front-end, and
