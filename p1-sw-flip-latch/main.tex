...
\item \textbf{Two measurements} (\cref{sec:measured}), bounded deliberately to
      the same guarantee class: readers of the transacted list stay within 1.4\%
      of plain \rcu{} from one reader to 192, while delivering the reverse walk
      plain \rcu cannot; a single writer pays 2.84 times, and a profile says
      where that goes --- 3.4\% of cycles in the commit, the rest in allocation
      and grace-period reclaim.
...
is not that. The write set is private to the transaction and to the thread that
owns it: no atomics, no lock, no line another thread is touching. For the
typical transaction, a two-edge list operation, it is a handful of records,
...
threshold, so a transaction that stays small never pays for one, and a long
...
ordinary uninstrumented reads, cheap and unguarded, and the transaction commits
...
% ---------------------------------------------------------------------------
% NOTE (drafting; retained deliberately -- see README framing rule).
% This section is an EXISTENCE RESULT and must stay one. Scope decided
% 2026-07-23 after McKenney asked for numbers: two measurements, same
% guarantee class only. Do NOT let an editing pass grow it into an evaluation
% -- no RLU/MV-RLU throughput (crosses guarantee classes, sec:relrcu says so),
% no writer scaling, no other structures. Those are the evaluation paper's.
% Every figure here is the mean of two 3s runs from scripts/p1_sw_list.csv in
% the benchmark tree; the profile figures are from a cycle profile of the
% single-writer panel. Cite no percentage that is not in one of those two.
% ---------------------------------------------------------------------------
...
bidirectional list of \cref{sec:swlist}. It is an existence result and not an
evaluation: two numbers, one per side, against plain \rcu only.
...
harness, under its \code{txn\_sw\_list} and \code{rculist} selectors, which name
the transacted list of \cref{sec:swlist} and Userspace \rcu's own
\code{cds\_list} respectively. The harness checks reader-visible coherence
on every traversal it performs; it reported no violation in any run reported
here.

\paragraph{Reads pay nothing for the transaction layer.} \Cref{tab:readcost}
gives the read ceiling with no writer running, in millions of node visits per
second, for the transacted list and for \code{rculist}.
...
second. The transacted list walks forward and then \emph{backward} --- the
operation \code{rculist} does not offer --- while \code{rculist} walks forward
twice, so both visit the same number of nodes and the comparison is per visit.}
\label{tab:readcost}
\centering
...
\midrule
\code{txn\_sw\_list} & 724 & 5\,824 & 23\,285 & 68\,141 & 136\,209 \\
\code{rculist}            & 732 & 5\,897 & 23\,576 & 68\,953 & 137\,466 \\
\midrule
Ratio                     & 0.989 & 0.988 & 0.988 & 0.988 & 0.991 \\
\bottomrule
\end{tabular}
\end{minipage}\par\medskip

The transacted list is never more than 1.4\% below plain \rcu{} --- the widest gap
is 1.39\% at two readers, and no count above two exceeds 1.25\%. Both scale
essentially linearly across the 192 cores: the transacted list gains a factor of
188 from one reader to 192, which is 98.0\% of ideal, and \code{rculist} gains
188, or 97.8\%.

That the two agree is the point, and the construction is what makes it a fair
statement. The transacted list is not doing the same work more slowly; it is
walking forward and then backward, which is the traversal \cref{sec:onestore}
showed \code{rculist} cannot offer at all, and it is being charged about one
percent for it. One percent is what \cref{tab:comparison}'s zero in the loads
column predicts: the marker rides in the word the reader had to load anyway, so
the only residue is the predicted branch, and a branch is what a percent looks
like.

\paragraph{The writer pays about three times.} On the same list, with a single
writer:

% Anchored, for the same reason as tab:readcost above.
\par\medskip\noindent\begin{minipage}{\linewidth}
\captionof{table}{Single-writer update throughput, millions of operations per
second. Both sides reclaim through \code{call\_rcu()} and neither changes \rcu
flavor, so the ratio isolates the transaction layer.}
\label{tab:writecost}
\centering
\begin{tabular}{@{}lrr@{}}
\toprule
 & No writer lock & Uncontended mutex \\
\midrule
\code{rculist}            & 21.8 & 20.2 \\
\code{txn\_sw\_list} &  7.2 &  7.1 \\
\midrule
Ratio                     & 3.03 & 2.84 \\
\bottomrule
\end{tabular}
...
The asymmetry between the two columns is itself informative. Dropping the mutex
gains plain \rcu 7.0\% and the transacted list 0.7\%: the transaction's own work
dominates an uncontended lock acquisition, where for plain \rcu the lock is a
visible share of a much cheaper update. The realistic figure for this facility is
therefore the mutex column, 2.84, because the single-writer contract
(\cref{sec:exclusion}) assumes the caller holds a lock over those slots in any
case.

\paragraph{Where the writer's cost goes.} A threefold writer tax invites the
conclusion that the flip is expensive. It is worth disposing of that, because the
profile says the opposite. In a cycle profile of the single-writer run the commit
...
\paragraph{What this does not measure.} Two absences are deliberate. There is no
comparison against RLU or MV-RLU: they give readers a coherent snapshot, and
\cref{sec:relrcu} has already said that comparing throughput across that
difference compares different guarantees.
...
what happens when a 10\,000-node traversal amortizes one acquisition to nothing, and it is why the honest read-side comparison in
...
asymmetry: the low bits are the ones an implementation can be held to. A
facility could still offer both and let the embedder choose.
...
settled may still be resolving it, and a table sized to a transaction's width
