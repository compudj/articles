...
traversal, and the silence flatters RLU rather than us. A \pseudotxn has no such
cost, for the same reason it has no per-element state: a resolved slot is the
node itself, so a reader compares two pointers with \code{!=}. The general point
