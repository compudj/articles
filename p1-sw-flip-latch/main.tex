...
operation, and there the asymmetry does not soften: a low tag is a small
constant, so clearing it folds into the addressing-mode displacement of the load
that follows and costs no instruction of its own, where a high tag never folds
and needs one instruction ahead of that load even as a single bit. This one falls
on the taken path, which \cref{sec:costcomparison} counts as bounded rather than
steady state --- and the facility does not collect it in any case, since it
clears the tag with a mask rather than subtracting it.
