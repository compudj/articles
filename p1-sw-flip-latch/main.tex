...
\Cref{sec:onestore} established the gap this engine exists to close: a
...
structures: a bidirectional list whose reverse walk becomes ordinary, and a hash
list.
...
the backlink ever entering the transaction. \emph{Deferring} the backlink store
...
Read-your-own-writes costs a find per recorded edit --- \emph{is this slot
already pending?} --- but not the quadratic scan that first suggests. A typical
transaction is small, a two-edge list operation, where scanning the handful of
pending records beats anything cleverer, and the engine does exactly that; only
when a write set grows large enough for the scan to dominate does it arm a Bloom
filter, which answers the find's common outcome --- a miss, the slot is new ---
in constant time. The filter is built lazily, on the first lookup that would
scan a long write set rather than on the record that crosses the threshold, so a
transaction that stays small never pays for one. An embedder whose slots are
distinct \emph{by construction} can skip the find altogether by declaring the
handle disjoint. The declaration is a contract on slot \emph{addresses}, not on the
...
% (urcu-txn-dev 10f22a76, d47aaae8, cced8fb0). BEFORE that,
...
% -----------------------------------------------------------------------
% NOTE (drafting; retained deliberately -- see README framing rule).
% UPDATED when the sw engine gained read-your-own-writes. The
...
\code{09a8b796bf933fbd3525a1bff0a96c411cd88ee0} (2026-07-18), on branch
