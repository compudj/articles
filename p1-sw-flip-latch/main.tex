...
is where a single-instant switch of both edges --- or, if the edit is split
...
A \pseudotxn's marker occupies tag bits of the slot. For a pointer slot those
bits cost nothing: a pointer to an aligned object leaves its low bits clear by
construction, so every pointer-shaped slot already wastes them, and the marker
uses only what alignment left idle (\cref{sec:alignment} sets out how many). A
slot holding a non-pointer has no such waste to spend, so there the marker is not
free --- a counter or a bitmap word gives up one bit, carrying 63 on a 64-bit
word rather than 64 (\cref{sec:nonpointer}). Recognizing the marker is then
\begin{center}
...
check. An engine that checked at install --- a compare-and-swap on the slot,
...
% the series relationship belongs. Do not reintroduce a concurrent-engine
...
The engine closes this with \emph{read-your-own-writes}. Two accessors form the
pair: a load that returns a slot's pending new value if this \pseudotxn has
already recorded one and its committed value otherwise, and a recording store
that \emph{fuses} a second edit of a slot onto the existing record --- keeping
the committed old, advancing the new --- rather than appending a duplicate. A
transaction holds at most one record per slot. The wrappers read every neighbour
...
disjoint. The declaration is a contract on slot \emph{addresses}: a bitmap, whose
