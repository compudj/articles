...
value. That allocator must be a real one, not a fixed table indexed by the
handle: a proxy must outlive a grace period, since a reader that loaded its
handle before the commit settled may still be resolving it. A fixed table would
have to recycle a slot while such a reader was still reading it, whereas a range
allocator keeps every proxy independently reclaimable --- which is what lets many
transactions be in flight, and lingering within a grace period, at once.
