...
back to copying a larger region, to a grace-period drain, or to omitting the
operation. Userspace RCU's list does the last: it ships no RCU-safe reverse
iterator, because
\code{cds\_list\_del\_rcu()} publishes its forward edge atomically and its back
edge with a plain assignment.
...
and to its new value after. But a reader is \emph{not} a \pseudotxn---each of
its loads resolves independently, so a traversal spanning several slots may
straddle a commit. There is no opacity and no multi-read snapshot; a reader gets
monotonicity, never new-then-old.

The weakening is the design. Isolation is paid for on the read side, in state
attached to every element and work on every dereference---and the read side is
what RCU exists to keep free. A \pseudotxn's marker instead rides in spare bits
of the slot itself, the word the reader had to load anyway, so it adds no
per-element state and no extra load, and vanishes when no mutation is in flight.
The cost moves to the writer, in the width of a commit, and we say where.
\end{abstract}
...
the cleanest evidence available that the problem is real. Userspace \rcu's
doubly-linked list publishes only its forward chain: in
\code{cds\_list\_del\_rcu()} the forward edge is committed by an atomic store,
while the back edge is a plain assignment --- no release, and correspondingly
the \code{prev} pointers are never \code{rcu\_dereference}d. The consequence is
not a subtle ordering nit but a semantic one: a removed node survives as a ghost
until its grace period elapses, so a reverse walker can keep stepping onto nodes
the forward chain has already dropped. The \code{prev} view trails the
\code{next} view. There is no barrier that fixes this, because the two chains
are changed by two stores and nothing makes them one event. So the library ships
forward iterators and no RCU-safe reverse walk. Nobody has to be persuaded the
gap matters: the missing API is the argument.
...
\item \textbf{A \pseudotxn model} (\cref{sec:model}): a frozen set of
      ...
      including the property that a reader is not a \pseudotxn, and the weaker
...
\item \textbf{A per-record tag scheme} (\cref{sec:tags}): the marker lives in
      ...
      makes the reader's cost zero. Because a \pseudotxn may span structures
...
\item \textbf{Two transacted structures} (\cref{sec:wrappers}): a bidirectional
      ...
      \code{rculist} declines to offer becomes ordinary --- and a hash list. We
      also give the boundary: composition is legal only across slot-disjoint
      edits, and we say what happens when it is not.
...
Where a property here holds only under exclusion, we say so.
...
\toprule
 & \textbf{Per-element} & \textbf{Extra} & \textbf{Reader} & \textbf{Publish} \\
 & \textbf{state} & \textbf{loads} & \textbf{guarantee} & \textbf{step} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Where each design pays for multi-pointer atomicity. Read-path counts
are additional \emph{memory references} per dereference in the common case, over
and above the load the reader must issue anyway; both existence and RLU do more
work while a mutation is in flight. A \pseudotxn issues none, because its marker
rides in the value \code{v} that \code{rcu\_dereference} already returned. The
first column is the unconditional cost: existence and RLU attach state to every
element for as long as the structure lives, whereas a \pseudotxn attaches
nothing and confines its state to the mutation. Existence's load is an
\emph{acquire}; RLU's is a plain load from its per-thread state. Ours is zero
because the marker rides in \code{v} itself, not because the check is cheap.
MV-RLU is the one entry whose count is not a constant: its dereference reads the
object header unconditionally --- it has no counterpart to RLU's pure-reader
bypass --- and then walks a version chain whose length the reader does not
control, so $\geq 2$ is a floor rather than a typical case.}
...
parties and gives each of them a different bargain. A reader brackets its
traversal in \code{rcu\_read\_lock()} and \code{rcu\_read\_unlock()}. On the
common implementations these are not locks at all: they exclude nothing, they
contend with nothing, and in the userspace library's \QSBR mode they compile to
nothing at all. What a reader gets in exchange for that non-cost is narrow but
sufficient: any object it reaches during the bracket remains allocated until the
bracket ends.
...
bracket has been observed to leave. Once one elapses, no reader can still hold a
...
interface of the most widely deployed \rcu container there is.

Userspace \rcu ships a circular doubly-linked list in two variants. The plain
one, \code{<urcu/list.h>}, iterates in both directions:
\code{cds\_list\_for\_each\_prev()},
\code{cds\_list\_for\_each\_prev\_safe()}, and
\code{cds\_list\_for\_each\_entry\_reverse()} are all part of its interface. The
\rcu one, \code{<urcu/rculist.h>}, ships exactly two iterators ---
\code{cds\_list\_for\_each\_rcu()} and
\code{cds\_list\_for\_each\_entry\_rcu()} --- and both walk forward. There is no
\rcu-safe reverse walk, and this is not because the backward chain is missing.
Every node still has a \code{prev} pointer and every writer still maintains it,
on \code{add}, on \code{del}, and on \code{replace}. The chain is there, it is
correct, and a reader may not read it.

The reason is one line. In \code{cds\_list\_del\_rcu()}, unlinking a node
\code{E} between \code{P} and \code{N} re-points two edges:

\begin{lstlisting}
/* unlink E, which sits between P and N */
elem->next->prev = elem->prev;                 /* N->prev: E -> P */
uatomic_store(&elem->prev->next, elem->next);  /* P->next: E -> N */
\end{lstlisting}

The forward edge is written with \code{uatomic\_store()}: indivisible, safe for
a concurrent reader to load. The back edge is written with a bare \code{=}, to a
node that readers can reach at that very moment. Under the C11 model a plain
store racing with a concurrent load is a data race and the behaviour is
undefined --- so that line is sound \emph{only because no reader is permitted to
read \code{prev}}. The missing iterator is not an oversight or a gap someone
neglected to fill. It is load-bearing. The library declines the operation, and
in exchange the write side gets to be cheap.

It is tempting to conclude that the omission is a mere ordering bug: publish
\code{prev} with \code{rcu\_assign\_pointer()}, read it with
\code{rcu\_dereference()}, and the reverse walk falls out. It does not, and
seeing why is the point of this subsection. Suppose both edges were properly
...
--- the operation the reverse iterator exists to serve --- gets an answer no
sequential list could ever produce.

Ordering does not fix this because ordering is not what is missing. What is
missing is \emph{atomicity across two slots}, and \rcu has no way to ask for it.
...
\label{def:pseudotxn}
A \pseudotxn is a set of records over pairwise-distinct slots, \emph{frozen}
at the commit point, together with a single \emph{commit step} that makes the
whole set take effect at once.
\end{definition}

Two properties of \cref{def:pseudotxn} carry the entire model. The record set
is \emph{frozen}: no record may join it after the commit step begins, so what
becomes visible is decided before anything is published. And the commit is a
\emph{single step}: one store, not a protocol, so there is no interval during
which part of the set has taken effect and part has not.
...
\begin{property}[Single linearization point]
\label{prop:lp}
A committed \pseudotxn is linearizable, and its linearization point is the
commit step. The steps that stage the set beforehand and that tidy up
afterwards change no observable value, so neither is a linearization point.
\end{property}
...
What may legally be folded into a single commit is constrained in turn:
composition is sound across slot-disjoint edits, which cross-structure
composition satisfies by construction, and \cref{sec:swcomposition} gives the
boundary and what happens on the wrong side of it.
...
\textbf{A reader is not a \pseudotxn.} It is a sequence of single-slot reads,
each linearizing at its own access. A traversal spanning several slots may
therefore \emph{straddle} a commit: it can observe the \pseudotxn in a slot it
reads after the commit and not in one it read before.
...
it is safe as stated and why it costs the reader nothing.
...
\paragraph{Isolation is paid for on the read side.} A snapshot is a claim about
what a reader observes, so only the reader can establish it. RLU establishes it
by giving every object a header that can name a newer copy, and by having
readers consult that machinery when a writer's copies might be relevant to them.
Existence structures establish per-element membership by giving every element a
field that can name a group, and having readers load it. In both cases the
mechanism must be reachable \emph{from the reader}, which is precisely why it
must live in, or hang off, the elements themselves.

\paragraph{Every other design adds a load; this one adds none.} The read-path
costs are ordered, and the ordering is structural rather than a matter of
constant factors. Existence's fast path is one acquire load of a per-element
field. RLU's fast path, for a thread with no writes of its own, never touches
the object header --- but it still loads a flag from its per-thread state, once per
dereference. Neither is expensive; both are memory references.

A \pseudotxn issues none. Its marker occupies tag bits of the slot, so
recognizing it is
\begin{center}
\code{((uintptr\_t) v \& tag) == tag}
\end{center}
where \code{v} is the value the reader's \code{rcu\_dereference} already
returned. That is arithmetic on a register the reader was obliged to hold
anyway: no additional load, no additional cache line, and a branch that is not
taken in the absence of an in-flight commit. The marker rides the dereference
the reader had to perform regardless.
...
\paragraph{A pseudo-transaction charges the reader nothing.} The engine adds no
per-element state at all. Its marker lives in tag bits of the slot itself ---
the very word the reader had to load in order to proceed --- so recognizing it
costs no additional load, no additional line, and a branch that is not taken
except in the presence of an in-flight commit. And because the marker is
transient, the structure a reader traverses in the steady state is
byte-for-byte the structure it would traverse with no engine present.
...
comparison against RLU crosses classes and we say so wherever we make one
(\cref{sec:related}).
...
set of installed proxies, frozen once installation ends, and the commit is the
selector store. It is also why \cref{prop:monotone} holds --- the selector is
written once, $0 \to 1$, and never written back.
...
\item \textbf{Install.} Initialize the group, then for each slot initialize a
      proxy with its $\{\mathit{old},\mathit{new}\}$ targets and point the slot
      at the tagged proxy. The selector is $0$, so this is \emph{transparent to
      readers}: they still resolve to old. Installation is not a partial commit;
      it is not observable at all.
...
% -----------------------------------------------------------------------
% NOTE (drafting; retained deliberately -- see README framing rule).
% PENDING (as of 2026-07-16): there is no SINGLE-WRITER bitmap wrapper yet.
% rcu-txn-bitmap.h is written against the multi-writer MCAS engine only, and
% the sw wrappers are the list and hlist. Mathieu plans to implement an sw
% bitmap. So Sec. 5.2 and 5.3 currently argue the capability from the TAG
% CONTRACT -- which is legitimate, because it IS a property of the contract
% and not of either engine -- but they cannot point at a shipped sw wrapper.
% When the sw bitmap lands, revisit both sections and Sec. 6: the prose is
% already written and needs only the wrapper as its concrete instance. Two
% things to check at that point: (1) whether rank()/select() consistency
% differs under exclusion -- the MCAS bitmap leans on the value-CAS at
% commit to re-validate the bits within a word, and the sw engine has no
% such CAS; (2) whether the sw bitmap keeps the narrow bit-0 tag, since Sec.
% 5.3's whole example depends on it composing with wider-tagged pointer
% slots in one commit.
% -----------------------------------------------------------------------
% -----------------------------------------------------------------------
% NOTE (drafting; retained deliberately -- see README framing rule).
...
\cref{tab:comparison}'s last row reads zero, and it is why the structure a
...
\subsection{Composition is slot-disjoint only}
\label{sec:swcomposition}

The \sw engine's composition has a hard limit, and it follows directly from what
\cref{sec:exclusion} removed.

The engine has no transactional loads and no same-slot reconciliation. Each
\code{*\_prepare} therefore reads its neighbour slots \emph{raw}, and cannot see
an edit made earlier in the same bracket. Composition is legal only across
\textbf{slot-disjoint} edits.

Cross-structure composition --- the intended use --- is disjoint by
construction: a node published into a trie and spliced into a list touches
slots that cannot coincide. But two edits on \emph{one} list whose
neighbourhoods touch, adjacent deletes being the obvious case, will silently
commit stale pointers. The second edit's idea of its neighbour predates the
first edit, and nothing in this engine notices.
% -----------------------------------------------------------------------
% NOTE (drafting; retained deliberately -- see README framing rule).
% Say "silently" and mean it -- there is no abort, no assert, no debug trap
% for this in the sw engine. It is the sharpest edge in the paper and it
% must be in Sec. 7 as well. This is also the cleanest possible motivation
% for the companion paper: the multi-writer front-end's read-your-own-writes
% chaining exists precisely to make same-slot composition safe, and it is
% the reason that engine has transactional loads at all. So the limitation
% is not a defect to apologise for -- it is the boundary that defines what
% the other engine is for. Frame it that way.
% -----------------------------------------------------------------------
...
\item[Distinct slots, and no raw access.] A \pseudotxn's records must target
  pairwise-distinct slots; every write to a transacted slot must go through the
  engine and every read through its accessor.
...
Slot-disjointness of composed edits (\cref{sec:swcomposition}) is likewise
unchecked in this engine, and its violation is the quietest of all: the second
edit records an old value that predates the first, and the commit publishes it.
No abort, no assertion, no debug trap.
% -----------------------------------------------------------------------
% NOTE (drafting; retained deliberately -- see README framing rule).
...
\textbf{No reader snapshot.} A reader is not a \pseudotxn
(\cref{sec:nonguarantees}); a traversal spanning slots may straddle a commit.
An embedder needing a mutual snapshot layers its own versioning on top, as with
any value-based \mcas. This is the deliberate weakening of
\cref{sec:whyweakening}, not a shortfall.
...
must carry its own version in the word. Under exclusion this has no teeth ---
nothing else can change the slot --- which is why it surfaces here only through
\cref{sec:swcomposition}: a stale recorded old is exactly a same-bracket edit
this engine cannot see.
% -----------------------------------------------------------------------
% NOTE (drafting; retained deliberately -- see README framing rule).
% Good unification worth keeping: in the single-writer engine, "value-based
% not stability-based" and "composition is slot-disjoint only" are THE SAME
% limitation seen from two sides. Under exclusion the only thing that can
% invalidate a recorded old is an earlier edit in your own bracket. In the
% concurrent engine they separate, and that is where the guard/validate API
% earns its place -- so the unification is a P1 observation, not a general
% one.
% -----------------------------------------------------------------------
...
set is atomic; a reader is not a \pseudotxn, and a traversal spanning several
...
targeted. Isolation is paid for on the read side, and the read side is what \rcu
exists to keep free.
...
\code{ec621268bba1f16711e992fa7320851d7a1a3205} (2026-07-14), on branch
...
\item[\code{rcu-txn-sw.h}] The \fliplatch: flip groups, proxies, resolution, and
      ...
      source of \cref{sec:monotone} and \cref{sec:publishorder}.
...
\noindent Two further headers are named here for orientation rather than as
contributions of this paper: \code{rcu-mcas.h} is the \mw engine taken up in the
companion paper, and \code{rcu-txn-bitmap.h} is a non-pointer encoding
(\cref{sec:nonpointer}) written against that engine rather than this one.
