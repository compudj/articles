...
states what it costs in return: the engine has nowhere of its own to put the
...
which is \cref{prop:crossstructure} put to work, and the reason even an
...
disjoint. The declaration is a contract on slot \emph{addresses}, not on the
...
the embedder must guarantee is never a legitimate value.
...
in the slot's own spare bits, so it adds no per-element field and no header at
all, and its cost vanishes when no mutation is in flight.
...
companion paper, and \code{rcu-txn-sw-bitmap.h} applies the same tag contract to
a non-pointer word --- outside this paper's pointer-focused scope.
