...
to concurrent readers, and readers pay nothing for it in the absence of updates.
...
load---a reader that meets one resolves through the proxy and the shared selector
instead. The bulk of the cost moves to the writer, in the width of a commit.
\end{abstract}
...
contract that makes \rcu worth using. Specifically:
...
read side is precisely what this facility declines to charge.
% -----------------------------------------------------------------------
...
\rcu exists because readers dominate. Its entire contract is that a reader pays
...
they need a different facility. The comparison holds where the guarantee classes
...
The facility closes this with \emph{read-your-own-writes}. Two accessors form the
...
pending records beats anything cleverer, and the facility does exactly that; only
...
architectures that offer fewer bits than it wants. A facility could offer both
...
cost this facility exists to avoid.
...
proxies resolve correctly forever, so a never-settling facility is \emph{correct}.
...
proxy record and then the group's shared selector, one or two loads the fast
...
comparison of throughput between this facility and RLU crosses guarantee classes.
...
We have described a fourth answer that leaves the contract intact. A \pseudotxn
...
contributions of this paper: \code{rcu-mcas.h} is the \mw facility taken up in the
