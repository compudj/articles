...
\draw[-latex, semithick] (7.0, 2.8) -- (7.0, 4.0) -- (9.25, 4.0);
\node[right, font=\scriptsize] at (7.05, 3.45) {\SUCCEEDED};

\draw[-latex, semithick, gray!60!black] (7.0, 2.0) -- (7.0, 0.6) -- (9.8, 0.6);
\node[right, font=\scriptsize, text=gray!45!black] at (7.05, 1.35) {\FAILED};

% ===== outcome 1: committed =================================================
\node[lnode] (cP) at ( 9.8, 4.0) {\code{P}};
\node[nnode] (cE) at (10.9, 4.0) {\code{E}};
\node[lnode] (cN) at (12.0, 4.0) {\code{N}};
\draw[biarr] (cP) -- (cE);
\draw[biarr] (cE) -- (cN);
\node[font=\scriptsize, text=green!35!black] at (10.9, 3.35)
  {\code{E} linked, both directions};

% ===== outcome 2: aborted ===================================================
\node[lnode] (aP) at (10.35, 0.6) {\code{P}};
\node[lnode] (aN) at (11.45, 0.6) {\code{N}};
\draw[biarr] (aP) -- (aN);
\node[font=\scriptsize, text=gray!45!black] at (10.9, -0.05)
  {unchanged: every record resolves to \textit{old}};
\node[font=\scriptsize, text=gray!55!black, align=center] at (10.9, -0.8)
...
same value $v$. The insert reads that slot but never writes it, so nothing else
in the transaction would notice a concurrent deletion of \code{N} marking it
(\cref{sec:guard}). On \SUCCEEDED\ every record resolves to its new value and
