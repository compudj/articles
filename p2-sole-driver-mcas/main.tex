...
version wide enough to be safe has to be imposed uniformly on all of them. And it
is not portably available---how many bits a word has to spare depends on the
architecture and on what the embedder stores there, so a scheme requiring several
per word constrains where it can run in a way a single tag bit does not. And the trade only arises at all under helping: the hazard the
...
operations racing: inserting a node after $A$, and deleting $B$. Whether the
facility's ordinary slot contention closes the race turns on a structural
question---how many slots the two operations \emph{write} in the adjacency they
share---and the doubly-linked list answers it in the safe direction.

Here the race is closed for free. Removing $B$ must rewrite $B$'s predecessor's
...
common slot after all---not by the tombstone. And because every adjacency in a
bidirectional list is held by two written edges, the same argument covers every
pair of adjacent operations on it: the structure is immune by construction.

That immunity belongs to the structure, not to the facility, and it lapses the
moment an adjacency is held by a single written edge. Two adjacent operations then
write genuinely disjoint sets of slots, both commit, and nothing tore---they simply
never met. This is where the hazard classically lives: in Harris's singly-linked
list a delete compare-and-swaps one node's forward pointer and an insert elsewhere
compare-and-swaps another's, and the two can miss each other
entirely~\citep{harris2001lists}. This paper's own hash list reaches that shape at
the tail.

Take the last node $X$ in a hash bucket. Inserting after $X$ writes one slot,
\code{X->next}: there is no successor, so there is no back edge to re-point and
the insert is a one-edge commit. Deleting $X$ rewrites the slot that names
it---its predecessor's \code{next}---and, $X$ being last, has no successor's back
pointer to fix either. Strip the tombstone out of that delete and the two write
sets are disjoint: the delete unlinks $X$ from the bucket while the insert hangs a
fresh node off \code{X->next}, and both succeed. The new node is unreachable and
the write landed in a node about to be reclaimed---a lost node and a write into
freed memory, from two commits that were each internally perfect.

What restores the meeting is the tombstone itself. The delete sets it on
\code{X->next}---precisely the slot the insert writes---so the two operations name
a common slot after all, with the same expected old value, and the facility again
commits at most one. That is the shape of the fix in general, and it needs two
things: a durable signal that a node is being deleted, set where a would-be
attacher must already look (\cref{sec:mark}), and a way to fold that check into
the attacher's own commit so its write is refused if the node died
(\cref{sec:structures}).
...
edge is installed into a node that is about to be reclaimed. The scope of this
second consequence is narrower than it first appears, and worth stating exactly.
Where the adjacency is held by two written edges---every adjacency of the
bidirectional list---\cref{sec:hazard}'s structural conflict has already refused
the attach, and the tombstone adds no safety the structure did not have. Where it
is held by one, as at the hash list's tail, the tombstone record \emph{is} the
shared slot, and it is the only thing that makes the two operations meet at all.
