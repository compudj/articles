...
sole-driver engine needs \textbf{no RDCSS}, and that the engine as a
...
\item \textbf{The sole-driver \mcas engine} (\cref{sec:engine}): a frozen record
      ...
      the owner alone. Its main result is a \emph{negative} one---the engine
      ...
      (\cref{sec:settle}). This is the first claim, and two elements of the second.
...
the engine: bit~0 in the structures shown here, a reserved nibble or high bits
...
clear. The engine's own fixed requirement is narrower and always met: a record's
...
comprehensive study is deferred to the evaluation paper.}%
% STALE TOO -- same vintage and same problem as the +21.7% figure; see the note
% in sec:varhelping. This one is a round "roughly 2.6", so it is likelier to
% survive a refresh than the two-decimal figures are, but it has not been
% re-measured on b3e23f9f either and must not be presented as if it had. So the more a
...
% ---------------------------------------------------------------------------
% NOTE (drafting; retained deliberately -- see README framing rule).
% THESE NUMBERS ARE STALE AND MUST BE REFRESHED BEFORE SUBMISSION.
%
% 39.9 vs 32.8 Mkey-moves/s (+21.7%, widening to +25.8%) comes from
% efficios-trie-benchmark scripts/txn_vs_existence_skiplist_engine.csv, captured
% 2026-07-10 (bench commit cce5bd3), when the engine tip was 97443472. This
% paper's Availability section pins b3e23f9f (2026-07-25). Fourteen engine
% commits separate them, and several are not cosmetic:
%
%   d5c5b2de  promote the mixed engine to the canonical front-end
%   5c156120  retire rcu-txn-mw.h, fold its tests onto rcu-txn.h
%   610d4792  skip the partition for a non-mixed write-set   <- commit path
%   6139087e  fold rcu-mcas.h in; retire the duplicate MCAS engine  <- THE engine
%   7be75ff6  re-verb the read policy from helping to waiting <- THE mechanism
%   b3e23f9f  resolve a proxy by subtracting the tag
%
% So the paper pins one engine and reports another's numbers. P1 had exactly
% this defect (text pinned b3e23f9f, tab:readcost came from 5fc3cadf) and it was
% fixed by re-measuring, not by re-pinning: re-pinning to the OLD commit would
% describe an engine nobody can now build from the shipped tree.
%
% REFRESHING IS NOT JUST RE-RUNNING. P1's re-measurement turned up two
% methodology faults, each of which moved the result by MORE than the effect
% being measured:
%   - traversal-order asymmetry between the arms (~15%), which had made the
%     facility look 8% faster than plain RCU when it is at parity;
%   - per-iteration loads of harness globals in some readers and not others,
%     which had made existence look like 2 extra loads per dereference when its
%     mechanism costs 1.
% Neither applies verbatim here -- bench_txn_3skiplist has no g_random_pos or
% STEP_LIMIT in its inner loop (checked) -- but the lesson does: before trusting
% a refreshed number, confirm the two arms do identical work apart from the
% mechanism under test.
%
% AND THE HARDER PROBLEM, which is why this is a note and not a chore. The
% helping engine is the arm being compared against, and it has been RETIRED
% from the tree (6139087e). A refresh therefore cannot just rebuild both arms
% at b3e23f9f. Either the helping engine is reconstructed at a pinned old commit
% for the comparison -- in which case say so, and say that the sole-driver arm
% is the old one too, so the pair is internally consistent -- or the claim is
% restated as what it actually is: a historical falsification, measured on the
% engine as it stood at 97443472, which is why helping is not in the shipped
% engine today. The second is honest and cheaper, and it is what the paper
% already argues (a measured falsification trail, sec:varhelping). It only needs
% the engine version stated beside the numbers rather than left to Availability.
% ---------------------------------------------------------------------------
...
The engine's descriptor-and-status shape
...
remaining ingredient of the engine is separately old: the descriptor-and-status
shape is Harris's~\citep{harris2002mcas, fraser2004lockfreedom}; that a
\emph{blocking} engine can beat a lock-free one is not our
...
\rcu's ordinary treatment~\citep{arbelraviv2017descriptors}; and logical deletion
is Harris's marked pointer and its descendants~\citep{harris2001lists,
ellen2010nbbst, brown2013scx}. What we have not found is their assembly into one
engine: RDCSS-free and sole-driven, \textbf{bounded-blocking with a cost-scaled
adaptive coarseness}, \textbf{refcount-free to reclaim} at the instant owner-only
settle makes a transaction unreachable, and \textbf{usable in place} through a
...
is the point, not a shortfall: we describe the engine, and the alternatives we
...
is not ours; folded into the engine it is one part of the paper's second claim.
...
descriptor-and-status engine, the bounded-blocking progress, the refcount-free
reclamation, the logical deletion---is borrowed, but their assembly into one
engine, with the no-RDCSS result as its linchpin, is a combination we have not
found before.
...
\item[\code{rcu-txn-mcas.h}] The sole-driver \mcas engine: records, the tri-state
      ...
      (\cref{sec:engine,sec:norddcss,sec:settle}). At the pinned commit this
      header carries both install disciplines --- the CAS plant described here
      and the exclusive park of the single-writer paper --- behind one
      descriptor and one status word.
