...
How much that costs is a question with no single answer, and the shape of the
answer is the argument. The lane is one global critical section, so its
throughput is flat in the writer count while the optimistic path scales; the
ratio between them is therefore a function of scale, and quoting it without one
says nothing. Measured on a three-structure skip-list move, optimistic against a
...
\midrule
optimistic $/$ lane & $1.0\times$ & $2.3\times$ & $25\times$ & $51\times$ &
  $98\times$ & $172\times$ & $303\times$ & $946\times$ & $1029\times$ \\
\bottomrule
\end{tabular}
\end{center}
...
funnel rather than itself---and by 192 writers the optimistic path is three
orders of magnitude ahead.\footnote{\code{bench\_txn\_3skiplist}, width~3, 3840
keys per skip list, median of three 2\,s runs, on the machine of
\cref{sec:varhelping}. The always-escalate arm forces the lane from the first
attempt (a zero budget); it is the limit case, not the shipped policy, which
escalates only a losing handle. The comprehensive study is deferred to the
evaluation paper.} So the more a
...
% the decisive number is the help+steal -> sole-driver A/B, now +22.1% at 192
% writers (to +27.0% under contention), from
...
$40.8$ against the helping engine's $33.4$ million key-moves per
second---$+22.1\%$---and the margin grows as the structures shrink and the
shared upper levels grow hotter, reaching $+27.0\%$; it also grows with the
...
width~3, jemalloc, median of five 2\,s runs, on a 2\,$\times$\,96-core EPYC 9654
(384 hardware threads). Run-to-run spread within a point is at most 2.3\%, so
...
compile-time flag, the install discipline. See \cref{sec:availability}.}
...
arm on both. At the disclosed commit it is between $0.5\%$ and $3.1\%$
...
% ---------------------------------------------------------------------------
% NOTE (drafting; retained deliberately -- see README framing rule).
% RE-MEASURED 2026-07-28.  The stale 2026-07-10 numbers (39.9 vs 32.8, +21.7%,
...
Helping (co-install) &
  ...
  and steal: \textbf{$+22.1\%$} at 192 writers, to $+27.0\%$ under contention &
...
removal ($+22.1\%$, \cref{sec:varhelping}) is the throughput evidence this paper
