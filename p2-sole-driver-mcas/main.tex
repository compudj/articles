...
\emph{validate the successor's tombstone}. Here the two structures make the
placement choice \cref{sec:mark} left open, and pack the tombstone into a
reserved bit of each node's own forward pointer. That is what makes one guard
enough: the word the insert must already consult to serialize against a deletion
is the same word that deletion marks, so a single validated read covers both. The
insert issues a \emph{load-validate} on the successor's forward pointer---it
reads the value and records a load-only guard, a record of the form
\edit{v}{v}---so that the commit succeeds only if that slot still resolves to $v$
at the install point. Were the tombstone a separate status word instead, nothing
about the argument would change except its address: the guard would name that
word, and the insert would carry two read-validated slots rather than one. If a concurrent deletion has set
