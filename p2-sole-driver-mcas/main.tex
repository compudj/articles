...
writers: a \pseudotxn is driven only by its owner. A writer needing a foreign
in-flight slot settled waits a bounded spin, then escalates; a reader resolves it
at once and never waits. There is no helping, no stealing, and---most of all---no RDCSS
...
argument is measured: a helping design was built and removed. It forced a
...
frozen node for free without consulting the tombstone, which leaves its
placement free. This is a known
...
reach lock-freedom. The paper states what it gives up and argues the trade.

The argument is not a preference stated after the fact. It is a falsification
trail. An earlier version of this facility had threads \emph{help} drive each
...
sequence and never new-then-old---for a dependency-chained traversal; the
companion paper's scope condition travels with that claim, and a reader that
compares two independently-reached slots owes itself an acquire.

Throughout this paper, \emph{transaction} is shorthand for that writer-side
\pseudotxn---a frozen record set published by one commit step---and never for a
read-and-write transaction in the database or software-transactional-memory
sense. Nothing on the read
side is transactional at all, which is the point \cref{sec:soledriver} turns on.
...
on a slot it does \emph{not} change. Where an adjacency is held by a single
written edge, deleting a node and inserting beside it touch \emph{different} words
and both succeed, and the result is a lost node or a use-after-free---not because
either commit tore, but because the two operations were never contending for a
common slot in the first place. Making a concurrent
...
that it \emph{cannot arise}, not that it is improbable. And the recurrence that
drives it is not some contrived churn on a word: it is what an ordinary pair of
structural edits does to a pointer.

Let a list hold $A \rightarrow B$, and let a transaction $T$ insert $X$ after
$A$---so $T$ carries a record $r$ naming the slot \code{A->next}, with old value
$B$ and new value $X$. Let $H$ be a thread helping $T$ install it.

\begin{enumerate}\setlength{\itemsep}{1pt}
\item $H$ reads \code{A->next}, finds $B$---the value $r$ expects---and prepares
      its compare-and-swap.
\item $H$ stalls before performing it: descheduled, preempted, anything.
\item $T$ is driven to completion by its owner or by other helpers. It commits
      and settles, leaving \code{A->next} holding $X$. The list reads
      $A \rightarrow X \rightarrow B$, and $T$ is retired.
\item Later, another transaction removes $X$. That restores \code{A->next} to
      $B$, and the list reads $A \rightarrow B$ once more.
\item $H$ resumes and performs its compare-and-swap. \emph{It succeeds}, because
      \code{A->next} does hold $B$.
\end{enumerate}

\noindent \code{A->next} now names $r$. A reader resolving it follows $r$ to
$T$'s status word, finds \SUCCEEDED, and takes $T$'s new value: $X$. \emph{The
removed node is back in the list.} Nothing was freed, and nothing could have
been: $H$ sits inside an \rcu read-side critical section for the whole of this,
because it is dereferencing a descriptor it reached through a slot, so no grace
period elapses and neither $T$'s descriptor nor $X$ is reclaimed while it is
stalled. \rcu forecloses the memory-safety reading entirely, and that is what
makes the real failure visible: the structure is simply \emph{incorrect}. A
deletion has been undone by a thread that was trying to help an insertion which
had already finished. This is why the hazard is A-B-A in the
structural sense and not merely in the arithmetic one---the pointer returned to
$B$ because a node was inserted and then removed, which is the most ordinary
thing a list does. RDCSS closes step~5 by conditioning the install on a second
word: the descriptor's status must still be \UNDECIDED, and $T$'s no longer is.

Now notice which step carries the weight. Steps~1, 3, 4 and~5 are
unremarkable---a read, an insertion completing, a removal, and a compare-and-swap
that finds exactly what it expected. None of them can be designed away, and the
recurrence in step~4 is not a rare interleaving but the normal life of a list.
Everything hinges on step~2: a thread that had resolved to install one of $T$'s
records and had not yet done so when $T$ finished. That thread cannot be $T$'s
owner. An owner plants its whole prefix and only then decides, so at the instant
$T$ becomes terminal its owner has no plant outstanding---there is nothing left
for it to stall before. The stalled installer of step~2 is therefore a
\emph{second} driver of $T$, and a second driver is what helping introduces and
what \cref{def:soledriver} forbids. The property states that.
...
its old value at the linearization point, not to have been stable throughout. An
embedder that needs stability across an interval layers its own versioning on
top, as with any value-based \mcas. Memory safety rests on the reclamation
obligation above, not on slot values: a value that recurs costs atomicity, never
safety.
...
over and slots it does not. Split those across two facilities---one assuming
exclusion, one assuming contention---and no single commit spans them, because a
slot can only be parked by the facility that manages it. The embedder is left
bridging the two with a lock of its own, which serializes the shared slots that
the multi-writer discipline exists to keep concurrent. This facility spans the
edit directly instead, by making the install discipline a property of the
...
give it. Let each discipline keep the representation that suits it---a proxy for
the exclusive one, a descriptor for the shared---and a single tag bit now names
two incompatible objects, so a reader meeting a tagged slot would have to know
which resolver to run before it could run one. That is a real obstacle, and it is
why carrying both disciplines in one facility is not the obvious arrangement.
Sharing the resolve header dissolves it: one layout, one resolver, and the
incompatibility turns out to be an artefact of representation rather than of the
problem.
...
regrouping only when a write set genuinely holds both. A uniform write set is
already grouped, so an all-shared commit pays nothing for the generality \emph{by
construction}, following the same path a facility offering only that discipline
would---a property read off the commit path, not a measurement; an
all-exclusive one skips the multi-writer machinery outright, through a second
commit entry point an embedder uses when it knows its write set carries no shared
record.
...
legitimate; concurrent disagreement is not. When in doubt, name it shared.

Within a single transaction the contradiction \emph{is} visible, and the facility
does not let it pass quietly. A debug build asserts on it where the second record
is staged, so an embedder that names one slot both ways in one transaction is
told at the point of the mistake; a release build resolves it in the safe
direction, promoting the slot to the CAS install rather than trusting a promise
the record set itself contradicts. The pairwise-distinct requirement above is
checked the same way, by a debug pass over the record set before installation
begins. It is worth being clear about which line those checks sit on. They cover
what one transaction can see about itself, which is why they can exist at all;
the global claim---that every other thread agrees about this slot---is not a
harder version of the same check but a different kind of statement, and no build
can make it. That one is discharged by the embedder's locking design or not at
all.
...
slot address. A committer waiting on slot $s$ therefore holds only slots below
$s$; the owner of $s$, installing in the same ascending order, is already past
$s$, so any slot \emph{it} waits on lies above $s$. Along any chain of waits the
contended addresses strictly increase, and a cycle would require them to come
back down. There is no cycle to close, so there is no deadlock.
...
records by address, buying the ordering the wait needs. A handle's \emph{age} is
simply how many times it has retried, and it selects between the two: age~0, the
first attempt, is unsorted and non-blocking; age~1 and above are sorted and
blocking.
...
wait even then. That concession is real. What the facility
...
conjunction of a bounded common-case wait and a starvation-free fallback: weaker
than lock-freedom in scheduler-independence, stronger than deadlock-freedom in
per-thread progress---the two axes the taxonomy separates, and which do not
...
That cost is not a fixed factor. The lane is a single global critical
...
single factor is only a reading taken at one point on that curve.

The measurement below uses a move across three skip lists, and the structure is
chosen rather than incidental. A skip list's upper levels are sparse, so every
writer descends through the same few nodes whatever key it is moving: contention
there is a property of the shape, not of an unlucky key distribution, and it does
not go away by making the structure larger. That is what makes it the case this
argument is about---a traversal mutator, costly to serialize because the descent
is most of what a lane would serialize, and with genuine conflict to resolve
rather than manufactured collisions. A structure whose writers rarely meet would
answer a different question, and answer it flatteringly. Optimistic against a
build that escalates on its first attempt:
...
\input{fig-writerscale}

\subsection{Helping, and the latch it forced}
\label{sec:varhelping}

The facility once helped. Threads that met an in-flight transaction drove it to
...
bought lock-freedom; it bought a \emph{more complex} bounded-blocking facility.
...
management, and it is well charted. The ``aggressive'' contention manager of
DSTM---the dynamic software transactional memory of Herlihy, Luchangco,
Moir and Scherer---aborts any conflicting transaction on
sight~\citep{herlihy2003dstm}, and the priority and
...
facility had it, in the form of \emph{stealing}: a higher-ranked transaction evicted
...
\emph{status}: stamp each transaction with a timestamp---an \emph{age} in the
older sense of the word, unrelated to the retry count of
\cref{sec:deadlock}---and let a reader or a committer decide a slot by comparing
those stamps instead of reading a status word. This
...
word, and that cost has two parts. It is an encoding budget, which this facility
deliberately declines to spend on the embedder's behalf: \cref{sec:constraints}
leaves the tag to each embedder precisely because slot encodings differ, and a
version wide enough to be safe has to be imposed uniformly on all of them. It is
also not portably available---how many bits a word has to spare depends on the
architecture and on what the embedder stores there, so a scheme requiring several
per word constrains where it can run in a way a single tag bit does not. And the
trade only arises at all under helping: the hazard the
...
Helping $+$ version-embedded words &
  needs spare bits in \emph{every} transacted word, so the facility would fix an
  encoding budget \cref{sec:constraints} leaves to each embedder, and how many
  bits exist to take is architecture- and embedder-dependent; buys freedom from a
  hazard a sole driver never admits (\cref{sec:varversion}). Independent
  concurrent work, not built here &
...
Whether the facility's ordinary slot contention closes a race between an insert
and a delete has a structural answer, and it is worth putting as a test one can
apply to any structure: \emph{does the write set of attaching something to a node
intersect the write set of removing that node from the reachable set?} Where the
two meet, the \kcas serializes the operations for free, on the shared slot, with
no help from any discipline layered above it. Where they are disjoint, both
commit, and nothing tore---the two operations simply never met.

A tree fails that test at every internal position, which is the cleanest place to
see the hazard. Let $C$ be a leaf with parent $P$, and let one thread grow the
tree at $C$ while another removes it:

\begin{quote}
\begin{tabular}{@{}ll@{}}
attach under $C$: & \code{C->child[i]}: \edit{\texttt{NULL}}{\texttt{newnode}} \\
delete $C$:       & \code{P->child[j]}: \edit{C}{\texttt{NULL}} \\
\end{tabular}
\end{quote}

Whether $C$ is reachable is witnessed \emph{above} it, by a slot in its parent;
the slot the attacher writes is \emph{below} it, inside $C$. Nothing brings the
two into contact, so both commits succeed: $C$ leaves the tree and is reclaimed
while \code{newnode} hangs beneath it, unreachable, written into a node that is
being freed. Parent pointers do not rescue it---the attacher's store into
\code{newnode}'s parent field is a plain store on a node still private to it, not
a record. This is the shape the non-blocking-tree lineage of
\cref{sec:tombstoneclaim} exists to handle, and it is why that lineage marks a
node with a word of its own rather than with a bit in a link: a tree has no slot
that both directions of an operation must already touch.

A doubly-linked list, by contrast, passes the test---and passes it twice over,
which is why it is a poor advertisement for the tombstone. Removing $B$ from
$A \leftrightarrow B \leftrightarrow C$ must rewrite $B$'s predecessor's forward
pointer, $A$'s \code{next}, the only forward pointer that names the path through
$B$; an insert after $A$ rewrites \code{A->next} too. Both compare-and-swap the
same slot with the same expected old value, so the facility commits at most one.
The loser fails its old-value check, aborts, re-reads, and re-descends into a list
that is still coherent; whichever wins, the outcome is well-formed. The back edge
is a second witness on top of that: an insert that attaches directly to $B$ still
re-points $B$'s successor's \code{prev}, and so does the delete. Every adjacency
is written from both directions, so the structure is immune by
construction---tombstone or no tombstone.

That immunity belongs to the structure, not to the facility, and it lapses as soon
as an adjacency is held by a single written edge. This is the classical home of
the hazard: in Harris's singly-linked list a delete compare-and-swaps one node's
forward pointer and an insert elsewhere compare-and-swaps another's, and the two
can miss each other entirely~\citep{harris2001lists}. This paper's own structures
reach that shape in exactly one place, the hash list's tail. Inserting after a
last node $X$ writes one slot, \code{X->next}---no successor, so no back edge to
re-point---and deleting $X$ rewrites its predecessor's \code{next} with, again, no
successor's back pointer to fix. Strip the tombstone out of that delete and the
two write sets are disjoint exactly as in the tree: the delete unlinks $X$ from
the bucket while the insert hangs a fresh node off \code{X->next}, and both
succeed.

What restores the meeting, in both cases, is the tombstone. The delete sets it on
a slot the attacher must already name---for the hash list, \code{X->next},
precisely the slot the insert writes---so the two operations share a slot after
all, with the same expected old value, and the facility again commits at most one.
That is the shape of the fix in general, and it needs two things: a durable signal
that a node is being deleted, set where a would-be attacher must already look
(\cref{sec:mark}), and a way to fold that check into the attacher's own commit so
its write is refused if the node died (\cref{sec:structures}).
...
\emph{Deletion is single-updater, and a retry can tell why it failed.} A second
delete of the node, or an insert or delete anchored to it, finds the tombstone set
and terminates with ``no such entry'' rather than acting on a node that is already
leaving. Exactly one transaction ever logically deletes a node, which is also what
lets a delete report whether \emph{this} call removed it, so two concurrent
deletes cannot double-free. This much the tombstone buys in every structure, and
it is the part nothing else supplies: an operation that loses a slot race learns
only that the slot moved. The tombstone is what separates ``a neighbour
moved---re-read and proceed'' from ``my anchor is gone---give up,'' and without
that distinction a retry loop has no correct way to terminate.
...
edge is installed into a node that is about to be reclaimed. How much this second
consequence is worth depends on the structure, and \cref{sec:hazard}'s test is what
decides it. Where attaching and removing already write a slot in common---every
adjacency of the bidirectional list---the structural conflict has refused the
attach on its own, and the tombstone adds no safety the structure did not have.
Where they do not, the tombstone record \emph{is} the shared slot and the only
thing that makes the two operations meet: in a tree that is every internal
position, which is why the discipline is not optional there; among the structures
shown here it is the hash list's tail.
That is also why the tombstone rides on a node's forward pointer and never on its
back pointer: no operation reaches a node solely through a back edge, so the
forward slots are both where every adjacency already serializes and where a retry
must read its answer.
...
the read side is byte-for-byte what it would be with no facility present, and
costs what the companion paper measured: a predicted branch, and parity within
...
list-specific machinery for contention; there is the sole-driver facility.

\subsection{The one guarded read}
\label{sec:guard}

Beyond the tombstone the structures need one more thing, and it is smaller than
the tombstone: a single guarded read. Being exact about what it does and does not
buy is worth a paragraph, because the obvious statement of it overshoots.

Start from what already serializes an insert against a deletion of its successor.
An insert after $P$ writes $P$'s \code{next} to name the new node; a deletion of
$P$'s successor $S$ rewrites that same slot to skip $S$. The two contend on
\code{P->next} with the same expected old value---\cref{sec:hazard}'s structural
conflict once more---so if the deletion commits first the insert fails its
old-value check there. That much needs no guard, and it is worth saying plainly:
an unguarded insert cannot \emph{commit} against a deleted successor.

What the guard changes is \emph{when} the insert finds out. The insert also writes
$S$'s back pointer, and the install can reach that slot before it reaches
\code{P->next}---records install in slot-address order once a transaction has aged
and in caller order before that, so either regime can put the back-edge install
first. The insert may therefore plant a record in a node whose deletion is about
to commit, and learn only at \code{P->next} that it must abort. Nothing incorrect
becomes visible: an installed record is not a committed store, and a reader that
resolves it gets the old value. But a transient record now sits in a node whose
delete has returned, and the aborting transaction's settle lands there later
still---which is exactly the straggler \cref{sec:reclaim}'s grace-period rule
exists to cover. Foreign slots from a composing structure, sitting between the two
addresses, widen the window further.

The guard closes that window at the source. The insert folds a
\emph{load-validate} of the successor's forward pointer---the slot a deletion of
$S$ marks---into its own write set: it reads the value and records a load-only
guard, a record of the form \edit{v}{v}, so the commit succeeds only if that slot
still resolves to $v$ at the install point. The back edge then serializes against
the deletion of $S$ exactly as the forward edge already did, and a marked
successor aborts the attempt before any record lands inside it. The guard also
gives the retry its answer, which is the harder half: a marked \emph{successor}
means a neighbour is leaving, which is transient and must be re-attempted, while a
marked \emph{anchor} is terminal---the distinction \cref{sec:mark} said a retry
loop cannot do without.

Here the two structures make the placement choice \cref{sec:mark} left open, and
pack the tombstone into a reserved bit of each node's own forward pointer. That is
what makes one guard enough: the slot the guard must name to serialize the back
edge is the successor's forward pointer, and that is the same word a deletion of
the successor marks, so a single validated read carries both duties. Were the
tombstone a separate status word instead, nothing about the argument would change
except its address: the guard would name that word, and the insert would carry two
read-validated slots rather than one. The guarded read is the read-side companion
to the tombstone: the forward edge is made safe by a \emph{write}-validated slot,
the back edge by a \emph{read}-validated one.
...
\input{fig-listinsert}

This is deliberately the minimal, functional form of a general facility. The
guard exists here for one reason---to protect the back-edge write of this paper's
own worked example---and it is presented as exactly that: a single
...
% back-edge dual of the mark. It must NOT claim the a-la-carte
% read-set-validation novelty (that is P3-1, whose novelty is the
% barrier-free-RCU substrate + opt-in-per-slot direction). Keep this
% subsection about "our list needs it", never about "here is a general
% facility". The exact source phrasing is that the load-validate of
% succ->next "exists ONLY to guard the pprev write".
%
% ADJUDICATED 2026-07-31 -- DO NOT REVERT TO THE STRONGER CLAIM.
% DECISION #5's premise ("without it P2's own flagship example is not correct
% under MW") is too strong, and the section no longer asserts it. Derivation
% from rcu-txn-list.h/rcu-txn-hlist.h: del(succ) rewrites &pos->next as its
% forward-unlink edge -- the SAME slot the insert writes with expected old
% value succ -- so an unguarded insert always fails its old-value check
% there. There is no interleaving in which it COMMITS against a deleted
% successor. The engine says so itself: the guard makes "the pprev side
% serialize against del(succ) exactly as @slot does", i.e. @slot/&pos->next
% is the durable serializer and the guard is early serialization.
% What the guard actually buys: (a) the -EAGAIN vs -ENOENT termination
% protocol, (b) closing the install window so no transient proxy/abort-settle
% lands in a node whose delete has returned (sec:reclaim's rule covers it if
% one does), (c) defence for the window widened by a composing structure's
% foreign slots. "Read-side companion to the tombstone" survives intact.
% If Mathieu finds a genuine both-commit interleaving, it belongs HERE as a
% worked trace -- it would be the strongest sentence in the subsection.
% -----------------------------------------------------------------------
...
the reason shows exactly what the kind contract forbids. Exclusive is a promise that the embedder
...
Keeping the storage \emph{valid} is not enough. The transacted-memory contract
of \cref{sec:limitations} is discharged by not freeing the node; a re-link
honours it to the letter---the memory is live, the word still means what it
meant---and loses the write anyway.

The pending write does not belong to the deleting transaction. By
\cref{sec:settle} no proxy of a transaction names any slot once its settle has
returned, and settle runs inside the commit, so a delete leaves nothing of its
own behind in the node it removed. The pending write belongs to a transaction
that \emph{lost} to it. Settle is a loop of plain stores over the planted
prefix, not one atomic operation. An insert that guards $B$'s forward pointer and
writes $B$'s back pointer (\cref{sec:structures}) holds a proxy in each; when it
aborts it converts them one at a time, and converting the forward pointer is
exactly what releases the delete that was contending for that slot. The delete
can then commit and its caller hand $B$ to reclamation while the loser's store
into $B$'s back pointer has not yet landed. Nothing serializes that last store
either: a delete rewrites its two neighbours' edges and the removed node's
forward pointer---never the removed node's own back pointer.

So a write into a removed node can land strictly after the operation that
removed it has reported success. An insert builds its new node's links with
plain stores while the node is private; if that storage is a recycled node, such
a store is overwritten when the straggler lands, and the write is silently lost,
leaving a permanently incoherent edge. Every straggler is inside the read-side
critical section its own attempt runs in, so one grace period drains them
exactly as it drains readers: a node's storage must wait out a grace period
before reuse exactly as its reclamation would---the same wait, extended from
``free'' to ``reuse.''
...
\paragraph{Transacted memory must be reclaimed through \rcu.} Here \rcu is an
\emph{existence} guarantee, not a courtesy to readers, and this is the contract
that makes concurrent transactions on an object possible at all. A transaction
writes each of its slots twice---a compare-and-swap at install, a plain store at
settle---and a transaction that \emph{loses} does both as well: it plants a
prefix, then settles that prefix back to the old values. An embedder that
unlinks an object and frees it immediately therefore leaves the loser
compare-and-swapping into freed memory. Every attempt runs inside a read-side
critical section, so deferring the free past a grace period is exactly what
closes this. The contract is that the backing memory of every transacted slot
outlives every transaction that can still name it---reclaimed through
\code{call\_rcu()}, or permanently allocated. \Cref{sec:reclaim} extends the
same wait from freeing to reuse.
...
as with any value-based \mcas. What an away-and-back change costs is atomicity,
not memory safety---memory safety rests on the lifetime contract above, not on
the value observed. Atomicity is conditional on the embedder's contracts---the
per-record tag kept free,
...
header shared by both kinds---which dissolves an apparent incompatibility
between the two disciplines rather than defending against it. It is
the same shape of result as the first claim, a mechanism turning out to be
unnecessary; it is weaker because we are not aware of anyone stating the
obstacle, and we claim it only as an element of this conjunction.
...
store, both in the same commit and both decided by the same status store. What
keeps two install disciplines in separate facilities is a difference in proxy
layout rather than a difference in the problem, and one shared resolve header
dissolves it---making the distinction writer-side and invisible to a reader,
which is what lets an embedder pay the blocking-progress price on exactly the
slots that are shared and on no others.
