...
forces, and no more. The comprehensive comparative evaluation---tail latency, and
comparison against existence, RLU, and MV-RLU---is later still. What appears here
is only the throughput that justifies a choice made \emph{in this paper}: a
design against the alternative we built and rejected, or the facility against
itself. \Cref{sec:writerscaling} is of the second kind and is the one figure the
paper's premise rests on; it compares this facility with the companion paper's
and with nothing else, which is why it belongs here and not with the
comparisons.
