...
The cost is countable rather than atmospheric: the latch put \emph{two extra
compare-and-swaps on every record}, one to claim the install
(\code{FREE}\,$\to$\,\code{BUSY}) before the record could be planted and one
to convert it at settle, which a helper might be doing concurrently and which
therefore could not be a plain store. The decision itself went the same way, from
...
measured. The latch is not merely state, it is \emph{atomics}: claiming a record
for install is a compare-and-swap (\code{FREE}\,$\to$\,\code{BUSY}), and
converting it at settle is another, because a helper may be converting the same
record at the same moment and a plain store would lose that race. So every
record carries two compare-and-swaps that the sole driver does not need, on top
of the one that installs it. The transaction's decision follows: the owner can no
