...
As in the companion paper, the claim is a conjunction, because each ingredient of
the engine is separately old. The \emph{descriptor-and-status shape}---a frozen
record set resolved through a status word---is
Harris's~\citep{harris2002mcas, fraser2004lockfreedom}. That a \emph{blocking}
engine can beat a lock-free one is not our
insight~\citep{david2013everything, hendler2010flatcombining}; the safety of a
disjoint-access operation only its owner may drive is Israeli and
Rappoport's~\citep{israeli1994disjoint}; the progress classes we place it in are
standard~\citep{herlihy2003obstruction, herlihy2011progress}, and its fair serial
lane is an MCS lock~\citep{mellorcrummey1991mcs}. Reclaiming descriptors after a
grace period is \rcu's ordinary treatment~\citep{arbelraviv2017descriptors}, and
logical deletion is Harris's marked pointer and its
descendants~\citep{harris2001lists, ellen2010nbbst, brown2013scx}.

What we have not found in that prior art is the combination: a multi-writer \mcas
engine that is \textbf{sole-driven and therefore free of RDCSS}---its install a
plain value-CAS made A-B-A-safe by construction rather than by a double-word
primitive, because the hazard RDCSS guards is helper-induced and there is no
helper---while being \textbf{bounded-blocking with a cost-scaled adaptive
coarseness}, \textbf{refcount-free to reclaim} at the instant owner-only settle
makes a transaction unreachable, and \textbf{usable in place} through a
logical-deletion discipline that charges the reader nothing. The linchpin, and the
one step we are not aware of drawn this way, is the first: removing helping makes
RDCSS \emph{unnecessary}, not merely cheaper. Every part is borrowed; assembling
them into an engine with that property is the claim---and, as a defensive
publication, we disclose it, and the alternatives we rejected
(\cref{sec:variations}), in enough detail to be rebuilt from these pages alone.
