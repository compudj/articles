...
% ---------------------------------------------------------------------------
A companion paper described an \rcu \pseudotxn---a frozen set of
...
\mcas engine realizes the same model for concurrent writers: a
transaction is driven only by its owner, and a thread that trips over an
in-flight transaction's proxy waits a bounded spin for that owner to settle,
then escalates. There is no helping, no stealing, and---this is the
centerpiece---no RDCSS. A plain value-CAS install is A-B-A-safe by
construction, because the re-plant hazard RDCSS exists to prevent needs a
\emph{foreign} driver of one's own records, which sole-driver excludes.
...
bidirectional list and hash list the companion paper used become usable under
concurrent writers.
\end{abstract}
...
Read-copy-update rests on a bargain: readers pay nothing, and in exchange a
...
is an engine, the sole-driver \mcas, and a claim about what \emph{kind} of engine
it should be.
...
transaction's records, which admits an install-time A-B-A hazard, which forced a
per-record \code{FREE}/\code{BUSY}/\code{DONE} install latch to close. That latch
...
\item \textbf{The sole-driver \mcas engine} (\cref{sec:engine}): a frozen record
      ...
      needs \textbf{no RDCSS}. A plain value-CAS install is A-B-A-safe by
      construction under one driver, because the re-plant hazard RDCSS exists to
      resolve requires a foreign driver of one's own records, which the regime
      excludes (\cref{sec:norddcss}).
...
\item \textbf{A measured design-space map} (\cref{sec:variations}): helping,
      abort-others, age-as-resolution, DCAS/RDCSS, and pure lock-freedom---each
...
records. A thread that meets a foreign in-flight record in a slot it cares about
waits a bounded spin for that record's owner to settle the slot to a plain
value, then proceeds.
\end{definition}

The discipline is total, and its most easily overlooked edge is the read side. A
load that lands on an \UNDECIDED foreign record does not help decide it. It waits
a bounded spin for the record's owner to settle---so that the value it returns is
stable---and if the owner is still undecided at the patience cap, it returns the
record's \emph{logical} value, the old, and moves on. The engine's own read
accessor states it as an imperative: \emph{never drive} the foreign
transaction. This closes an objection a careful reader will otherwise
...
%     competitive.
% OPEN QUESTION for Mathieu: this lever is not in the pinned engine at all.
% Options -- (a) keep it as an explicit future/defensive-disclosure subsection
% (current), clearly marked unimplemented; (b) cut it to a one-line forward
% reference; (c) hold it for the evaluation paper. Also owes the schedctl/TNP
% lineage cites (Edler 1988; Marsh 1991; Anderson activations 1991; Dice-Harris
% TRANSACT 2016) -- currently \NEEDCITE below.
% ---------------------------------------------------------------------------
...
not stall is textbook \NEEDCITE{schedctl (Solaris); temporary non-preemption
(Marsh--Scott--LeBlanc--Markatos 1991); scheduler activations
(Anderson 1991); Dice--Harris preemption-tolerance survey, TRANSACT 2016}. What
...
%
% MEASUREMENT GAP (SCOPE DECISION #1 + TASK): the decisive throughput numbers
% behind helping / abort-others / age-resolution live in the benchmark repo
% (efficios-trie-benchmark), NOT the engine headers. They are \UNVERIFIED
% placeholders below. DO NOT invent them -- locate the real runs before any
% arxiv build. The engine header confirms only the QUALITATIVE claim
% ("the sole-driver regime won across every measured workload").
% ---------------------------------------------------------------------------
